...
function ConceptB() {
  ...
  return (
    <div className="flex items-center justify-center h-full bg-gradient-to-br from-[#0f2a5e] via-[#1c3a6e] to-[#1a4f8a]">
      <div className="flex flex-col items-center gap-8 text-white text-center select-none">
        <img
          src="/icons/img_WF_Logo.png"
          alt="Seegene"
          className="w-64 brightness-0 invert opacity-90 pointer-events-none"
        />
        <div className="flex flex-col items-center gap-1">
          <p className="text-3xl font-light tracking-widest tabular-nums">
            {now.format('HH:mm:ss')}
          </p>
          <p className="text-sm text-blue-200">{now.format('YYYY년 MM월 DD일 dddd')}</p>
        </div>
        <div className="h-px w-48 bg-white/20" />
        <p className="text-sm text-blue-100">
          안녕하세요,{' '}
          <span className="font-semibold text-white">{user?.userName ?? 'DT관리자'}</span>님
        </p>
      </div>
    </div>
  )
...
function ConceptF() {
  ...
  const { setActiveMenu } = useAppStore()
  return (
    <div className="h-full bg-[#030507] overflow-auto">
      <div className="max-w-3xl mx-auto px-10 py-10">
        {/* 상태 표시줄 */}
        <div className="flex items-center justify-between mb-10">
          ...
          <span className="text-xs text-[#2A4A55] tracking-widest tabular-nums">
        ...
        {/* 사용자 */}
        <div className="mb-10">
          <p className="text-xs text-[#2A4A55] uppercase tracking-[0.3em] mb-3">OPERATOR</p>
          <h1 className="text-5xl font-light text-[#E8F4F8] leading-none tracking-tight">
            {user?.userName ?? 'DT관리자'}
          </h1>
          <div
        ...
        <div className="grid grid-cols-3 gap-4 mb-10">
          {[
            ...
            { label: 'REPORTED', value: '158', color: '#E8F4F8', border: 'rgba(232,244,248,0.1)' },
          ].map((s) => (
            <div
              key={s.label}
              className="rounded px-6 py-6 bg-white/[0.02]"
              style={{ border: `1px solid ${s.border}` }}
            >
              <p className="text-[10px] text-[#3A6A7A] tracking-[0.25em] mb-5 font-medium">
                {s.label}
              </p>
              <p className="text-5xl font-light tabular-nums" style={{ color: s.color }}>
                {s.value}
              </p>
              <p className="text-xs text-[#2A4A55] mt-2 tracking-widest">CASES</p>
            </div>
          ))}
        </div>
        ...
        {/* 빠른 메뉴 */}
        <div>
          <p className="text-[10px] text-[#2A4A55] uppercase tracking-[0.3em] mb-4 font-medium">
            QUICK · ACCESS
          </p>
            ...
            {QUICK_MENUS_F.map((m) => {
              ...
              return (
                <button
                  key={m.id}
                  onClick={() => setActiveMenu(m.id)}
                  className="group flex flex-col items-center gap-2.5 p-3.5 rounded border border-[#00D4FF]/8 bg-[#00D4FF]/[0.03] hover:bg-[#00D4FF]/10 hover:border-[#00D4FF]/25 transition-all"
                >
                  <Icon
                    size={16}
                    className="text-[#3A6A7A] group-hover:text-[#00D4FF] transition-colors"
                  />
                  <span className="text-xs text-[#3A6A7A] group-hover:text-[#7BAFC0] text-center leading-tight transition-colors">
                    {m.label}
                  </span>
                </button>
              )
