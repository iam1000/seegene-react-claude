...
const CONCEPT_KEYS = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I'] as LayoutConcept[]
...
function CompactBar2() {
  ...
  const { activeMenuId, setActiveMenu, layoutConcept, setLayoutConcept } = useAppStore()
    ...
    <div className="relative shrink-0" onMouseLeave={() => setHoveredMenu(null)}>
      ...
      >
        ...
        >
          ...
          <div
            className={`flex items-center gap-0.5 rounded-full px-1 py-0.5 transition-colors duration-200 ${
              isOpen ? 'bg-gray-100' : 'bg-white/6'
            }`}
          >
            {CONCEPT_KEYS.map((c) => (
              <button
                ...
                className={`px-2 py-0.5 rounded-full text-[9px] transition-all whitespace-nowrap ${
                  layoutConcept === c
                    ? isOpen
                      ? 'bg-gray-800 text-white'
                      : 'bg-sky-400/25 text-sky-300 font-medium'
                    : isOpen
  ...
  return (
    <div className="flex items-center h-9 bg-[#0F172A] border-b border-white/8 text-[11px] shrink-0 select-none">
      ...
      <div className="flex items-center gap-2.5 px-3 border-l border-white/8 shrink-0">
        ...
        <div className="flex items-center gap-0.5 bg-white/6 rounded-full px-1 py-0.5">
          {CONCEPT_KEYS.map((c) => (
            <button
              key={c}
              onClick={() => setLayoutConcept(c)}
