...
;((window, document) => {
  ...
  DOP.MakeDragObj = function (object, ev, T) {
    ...
    this.dy =
      parseInt(getComputedStyle(target).marginBottom) / 2 < 4
        ? 4
        : parseInt(getComputedStyle(target).marginBottom) / 2
    ...
    var adjVal = 0
    if (
      cloneObj.firstChild.tagName == 'INPUT' &&
      cloneObj.firstChild.getAttribute('type') == 'checkbox'
    ) {
    ...
    if (
      ev.target &&
      ev.target.childNodes[1] &&
      !(
        ev.target.childNodes[0].tagName == 'INPUT' &&
        ev.target.childNodes[0].getAttribute('type') == 'checkbox'
      )
    ) {
  ...
  DOP.MoveDragObj = function (ev, T) {
    ...
    if (this.tag) {
      ...
      if (
        isInside(rectRow, ev.x, ev.y) ||
        isInside(rectCol, ev.x, ev.y) ||
        isInside(rectData, ev.x, ev.y) ||
        isInside(rectFilter, ev.x, ev.y)
      ) {
        if (ev.target.tagName != 'DIV') {
          ...
          if (
            target &&
            this.orgTag != target &&
            (selfId != targetId || isInside(rectData, ev.x, ev.y))
          ) {
  ...
  Fn.makeChart = function (chartType, categories, datas, extraOpt) {
    var options = {
      ...
      tooltip: {
        headerFormat:
          DLG.Tag.querySelector('#_none') && DLG.Tag.querySelector('#_none').checked
            ? ''
            : '{series.name}<br/>',
      },
    }
    ...
    if (chartType && chartType.indexOf('column') === 0) {
      ...
      datas[0].data = datas[0].data.map((d, idx) => [categories[idx], d])
    ...
    // chart 생성
    createIBChart(doc.querySelector('.' + this.Style + 'ChartChart'), 'DLGChart', {
      width: '100%',
      height: '100%',
    })
    if (datas) for (var i = 0; i < datas.length; i++) options.series.push(datas[i])

    DLGChart.setOptions(options, { redraw: true })
  ...
  Fn.showChartDialog = function (chartType, extraOpt) {
    ...
    if (typePreset.indexOf(chartType) == -1) {
      this.showMessageTime(
        this.Lang.Dialog ? this.Lang.Dialog.ErrMsg.InvaildType : 'Invalid Type Error',
      )
      return
    }
    ...
    styles.textContent =
      '.' +
      classPrefix +
      'ChartPopup *{font-size:0.8rem;}' +
      '.' +
      classPrefix +
      'ChartPopup p{margin: 0;}' +
      '.' +
      classPrefix +
      'ChartPopup{' +
      ...
      '.' +
      classPrefix +
      'ChartExtraBtn>button.Zoom.active{' +
      ...
      '.' +
      classPrefix +
      'ChartExtraBtn.active>button.toggleDown{' +
      ...
      '.' +
      classPrefix +
      'ChartExtraBtn.active>button.btnDown{' +
      ' max-width: none;' +
      ' height: auto;' +
      ' margin-left: 8px;' +
      ' margin-right: 4px;' +
      ...
      'ChartPopup .' +
      classPrefix +
      'ChartConfig>ul>li>button.active, .' +
      classPrefix +
      'ChartPopup .' +
      classPrefix +
      'ChartConfig>ul>li>button:hover{' +
      '  background-color: ' +
      _ibThemePrimary[classPrefix] +
      ';' +
      '  color: #EEEEEE;' +
      '}' +
      '.' +
      ...
      '  border-bottom: 1px solid var(--chart-primary-' +
      classPrefix +
      ');;' +
      '}' +
      '.' +
      classPrefix +
      'ChartAccHeader:after {' +
      "  content:'';" +
      '  position: absolute;' +
      '  display: block;' +
      '  width: 10px;' +
      ...
      'div#tab1Con .' +
      classPrefix +
      'ChartAccContent ul{list-style-type:none;display:flex;justify-content:space-around;padding:0}' +
      'div#tab2Con .' +
      classPrefix +
      'ChartAccContent ul, div#tab1Con .' +
      classPrefix +
      'ChartAccContent ul{' +
      'padding:0;' +
      'margin:0;' +
      ...
      '#tab3Con .' +
      classPrefix +
      'ChartAccContent:not(.palette), #tab2Con .' +
      classPrefix +
      'ChartAccContent li, #tab2Con .' +
      classPrefix +
      'ChartAccContent div.__checkAllDiv{' +
      '  display: flex;' +
      '  align-items: center;' +
      '}' +
      '#tab2Con .' +
      ...
      '#tab2Con .' +
      classPrefix +
      'ChartAccContent div.__checkAllDiv label{' +
      '  margin-right: 0px !important;' +
      '  margin-left: 4px;' +
      '}' +
      '.' +
      classPrefix +
      'ChartAccContent:not(.palette) input[type=radio],.' +
      classPrefix +
      'ChartAccContent:not(.palette) input[type=checkbox] {' +
      '  margin:0' +
      '}' +
      'div:not(#tab1Con) .' +
      classPrefix +
      'ChartAccContent:not(.palette) label{' +
      '  margin-left: 4px;' +
      '  margin-right: 8px;' +
      '}' +
      '.' +
      classPrefix +
      'ChartAccContent.palette [type=radio]{position: absolute;opacity: 0;width: 0;height: 0;}' +
      '.' +
      classPrefix +
      'ChartAccContent.palette [type=radio] + div {' +
      '  cursor: pointer;' +
      '}' +
      '.' +
      classPrefix +
      'ChartAccContent.palette [type=radio]:checked + div {' +
      '  outline: 3px solid #7FFF00;' +
      '}' +
      '.' +
      classPrefix +
      'ChartAccContent.palette>label {' +
      '  width: 100%;' +
      '  margin-bottom: 6px;' +
      '}' +
      '.' +
      classPrefix +
      'ChartAccContent.palette>label:last-child {' +
      '  margin-bottom: 0px;' +
      '}' +
      '.' +
      classPrefix +
      'ChartAccContent.palette>label>div {' +
      '  display:flex;' +
      '  justify-content:space-between;' +
      '  height:20px;' +
      '  border-radius: 10px;' +
      ...
      '.' +
      classPrefix +
      'ChartAccContent.palette>label>div:last-child {' +
      '  margin-bottom: 0px;' +
      '}' +
      '.' +
      classPrefix +
      'ChartAccContent.palette>label>div>div {' +
      '  flex-grow:1' +
      '}' +
      '.' +
      classPrefix +
      'ChartAccContent.palette>label>div>div:first-child {' +
      '  border-top-left-radius:10px;' +
      '  border-bottom-left-radius:10px;' +
      '}' +
      '.' +
      classPrefix +
      'ChartAccContent.palette>label>div>div:last-child {' +
      ...
      'div#tab1Con .' +
      classPrefix +
      'ChartAccContent [type=radio] {' +
      '  position:absolute;opacity:0;width:0;height:0;' +
      '}' +
      'div#tab1Con .' +
      classPrefix +
      'ChartAccContent [type=radio] + img{' +
      '  cursor: pointer;' +
      '}' +
      'div#tab1Con .' +
      classPrefix +
      'ChartAccContent [type=radio]:checked + img {' +
  ...
  */
      ...
      ':root{' +
      '--chart-color-1: ' +
      _chartColorsArray[0][0] +
      ';' +
      '--chart-color-2: ' +
      _chartColorsArray[0][1] +
      ';' +
      '--chart-color-3: ' +
      _chartColorsArray[0][2] +
      ';' +
      '--chart-color-4: ' +
      _chartColorsArray[0][4] +
      ';' +
      '--chart-primary-' +
      this.Style +
      ': ' +
      _ibThemePrimary[this.Style] +
      ';' +
      '}' +
  ...
  **/
    var charts = [
      {
        Name: this.Lang.Dialog.ChartType.LineType,
        Menu: 1,
        Items: [
      ...
      {
        Name: this.Lang.Dialog.ChartType.ColumnType,
        Menu: 1,
        Items: [
      ...
      {
        Name: this.Lang.Dialog.ChartType.BarType,
        Menu: 1,
        Items: [
      ...
      {
        Name: this.Lang.Dialog.ChartType.PieType,
        Menu: 1,
        Items: [
      ...
      {
        Name: this.Lang.Dialog.ChartType.AreaType,
        Menu: 1,
        Items: [
      ...
      {
        Name: this.Lang.Dialog.ChartType.ComplexChart,
        Menu: 1,
        Items: [
    ...
    var body =
      ...
      "      <li class='" +
      classPrefix +
      "ChartTab active' id='tab1'>" +
      "        <button  type='button' class='active'>" +
      this.Lang.Dialog.ChartTypes +
      '</button>' +
      '</li>' +
      "<li class='" +
      classPrefix +
      ...
      "<div class='" +
      classPrefix +
      "ChartTabCon' id='tab1Con' style='display:block'>"
    for (var i = 0; i < charts.length; i++) {
      ...
      for (var x = 0; x < charts[i].Items.length; x++) {
        body +=
          '<li>' +
          "<label title='" +
          charts[i]['Items'][x]['Name'] +
          "' for='" +
          charts[i]['Items'][x]['Value'] +
          "'>" +
          "<input type='radio' id='" +
          charts[i]['Items'][x]['Value'] +
          "' name='chartType'/>" +
    ...
    body +=
      ...
      classPrefix +
      "ChartAccHeader'>" +
      this.Lang.Dialog.Category +
      '</div>' +
      "      <div class='" +
      classPrefix +
      "ChartAccContent categories'>" +
      '        <ul>' +
      '        </ul>' +
      '      </div>' +
      '    </div>' +
      ...
      classPrefix +
      "ChartAccHeader'>" +
      this.Lang.Dialog.Datas +
      "<div class='" +
      classPrefix +
      "ChartAccHeaderToggle'><label for='_allData'>" +
      this.Lang.Dialog.SelectAll +
      "<span class='_toggle'></span></ label><input type='checkbox' id='_allData'></div></div>" +
      "      <div class='" +
      classPrefix +
      "ChartAccContent datas'>" +
      '        <ul>' +
      '        </ul>' +
      '      </div>' +
      '    </div>' +
      ...
      classPrefix +
      "ChartAccHeader'>" +
      this.Lang.Dialog.UseTooltipDataLabel +
      '</div>' +
      "      <div class='" +
      classPrefix +
      "ChartAccContent'>" +
      "        <input type='checkbox' id='useToolTip' checked/>" +
      "        <label for='useToolTip'>" +
      this.Lang.Dialog.Tooltip +
      '</label>' +
      "        <input type='checkbox' id='useDataLabel'/>" +
      "        <label for='useDataLabel'>" +
      this.Lang.Dialog.DataLabel +
      '</label>' +
      '      </div>' +
      '    </div>' +
      "    <div class='" +
    ...
    body +=
      ...
      "              <label for='pivotData'>" +
      this.Lang.Dialog.SwitchCategory +
      '</label>' +
      '            </div>' +
      '          </div>' +
      '        </div>' +
    ...
    DLG.Tag.querySelector('.' + classPrefix + 'MenuBody').style.setProperty(
      'margin',
      '0',
      'important',
    )
    ...
    extraBtn.innerHTML =
      "<button type='button' class='toggleDown' title='" +
      this.Lang.Dialog.FileDownload +
      "'></button>" +
      "<button type='button' class='btnDown' title='" +
      this.Lang.Dialog.PngDown +
      "'>.png</button>" +
      "<button type='button' class='btnDown' title='" +
      this.Lang.Dialog.XlsxDown +
      "'>.xlsx</button>" +
      "<button type='button' class='Config' title='" +
      this.Lang.Dialog.Config +
      "'></button>" +
      "<button type='button' class='Zoom' title='" +
      this.Lang.Dialog.Zoom +
      "'></button>"
    ...
    for (var i = 0; i < accItems.length; i++) {
      ;((index) => {
        accItems[index]
          .querySelector('.' + classPrefix + 'ChartAccHeader')
          .addEventListener('click', () => {
            if (event && (event.target.tagName == 'INPUT' || event.target.tagName == 'LABEL'))
              return
            if (
              accItems[index].querySelector('.' + classPrefix + 'ChartAccContent').style.display ==
              'none'
            ) {
              accItems[index].querySelector('.' + classPrefix + 'ChartAccContent').style.display =
                accItems[index].parentElement.id != 'tab3Con' ? 'block' : 'flex'
            } else {
              accItems[index].querySelector('.' + classPrefix + 'ChartAccContent').style.display =
                'none'
            }
    ...
    for (var i = 0; i < chartInput.length; i++) {
      chartInput[i].addEventListener('click', () => {
        var doc = this.Document != document ? this.Document : document
    ...
    DLG.dialogResize = (e) => {
      ...
      resizeTimer = setTimeout(() => {
        if (DLG.Type == 'Chart' && DLG.Tag) {
          DLG.Tag.firstChild.style.width =
            DLG.isZoomIn || window.innerWidth < 700 ? 'calc(100vw - 20px)' : '700px'
          var size = DLG.Tag.getBoundingClientRect()
          if (
            DLG.Tag.querySelector('button.Config') &&
            DLG.Tag.querySelector('button.Config').classList.contains('active')
          )
            size.width =
              size.wdith - DLG.Tag.querySelector('div.' + classPrefix + 'ChartConfig').offsetWidth
  ...
  Fn.makeDataForChart = function () {
    ...
    var selectRows =
      selectRange.length === 0 ? this.getDataRows(1, 1) : this.getSelectedRows(0, 'Visible')
    ...
    var categories = selectRows
      .filter((r) => r.Name != 'SubSum')
      .map(function (r) {
    ...
    var datas = []
    var selectCols = cols
      .filter((c) => cols.indexOf(c) >= startColIdx && cols.indexOf(c) <= lastColIdx)
      .filter(function (c) {
        return (
          (this.Cols[c]['Type'] == 'Int' || this.Cols[c]['Type'] == 'Float') && c != this.RowIndex
        )
      }, this)
  ...
  Fn.DialogButtonsConfig = function (DLG) {
    ...
    DLG.imageDown = function (returnImg) {
      ...
      blobToBase64(blob, (blobURL) => {
        var download = (href, name) => {
    ...
    DLG.excelDown = function () {
      ...
      var imgIntv = setInterval(() => {
        if (DLG.tempImgInfo) {
          clearInterval(imgIntv)
          var sheet = DLG.sheet
          var param = {},
            categoryCol = doc.querySelector(
              '.categories input[type=radio][name=category]:checked',
            ).id

          param.downRows = sheet
            .getSelectedRows(0, 'Visible')
            .map((r) => sheet.getRowIndex(r))
            .join('|')

          if (categoryCol != '_none') {
            param.downCols = [categoryCol]
              .concat(
                Array.prototype.slice
                  .call(DLG.Tag.querySelectorAll('#tab2Con .datas input:checked'))
                  .map((input) => input.id),
              )
              .join('|')
          }
              ...
              Cells: [
                {
                  ...
                  Value:
                    '|' +
                    DLG.tempImgInfo.path +
                    '|' +
                    DLG.tempImgInfo.width +
                    '|' +
                    DLG.tempImgInfo.height,
                  ShrinkToFit: false,
                },
              ],
            },
    ...
    DLG.makeTab2 = function (isNew) {
      var chartType = DLG.Tag.querySelector(
        '.' + this.sheet.Style + 'ChartTabCon#tab1Con input[type=radio]:checked',
      ).id
      ...
      var CATEGORY = [
        "<li><input type='radio' name='category' id='_none'><label for='_none'>" +
          this.sheet.Lang.Dialog.None +
          '</label></li>',
      ]
      var selectedCols
      if (!isNew) {
        selectedCols = Array.prototype.slice
          .call(
            DLG.Tag.querySelectorAll(
              '.' + this.sheet.Style + 'ChartTabCon#tab2Con .datas  ul input:checked',
            ),
          )
          .map((input) => input.id)
      }
      if (isCombi) {
        ...
        for (var c = 0; c < cols.length; c++) {
          ...
          if (type == 'Int' || type == 'Float') {
            ...
            CATEGORY.push(
              "<li><input type='radio' name='category' id='" +
                cols[c] +
                "'><label for='" +
                cols[c] +
                "'>" +
        ...
        DLG.Tag.querySelector('.datas').innerHTML =
          "<div class='combinationDiv'>" +
          "<div class='cls1'>" +
          // +"  <div>"+cls1.Name+"</div>"
          "  <div class='__checkAllDiv'>" +
          "<input type='checkbox' id='checkAll_____1' class='checkAll' name='checkAll'><label for='checkAll_____1'>" +
          cls1.Name +
          '</label>' +
          '</div>' +
          '  <ul>' +
          ...
          "<div class='cls2'>" +
          "  <div class='__checkAllDiv'>" +
          "<input type='checkbox' id='checkAll_____2' class='checkAll' name='checkAll'><label for='checkAll_____2'>" +
          cls2.Name +
          '</label>' +
          '</div>' +
          '  <ul>' +
      ...
      if (categoryRadio.length > 1) {
        if (categoryCol && DLG.Tag.querySelector('#' + categoryCol))
          DLG.Tag.querySelector('#' + categoryCol).setAttribute('checked', 'true')
        else categoryRadio[categoryRadio.length - 1].setAttribute('checked', 'true')
        DLG.Tag.querySelector('.categories>ul>li').parentNode.removeChild(
          DLG.Tag.querySelector('.categories>ul>li'),
        )
      } else DLG.Tag.querySelector('#_none').setAttribute('checked', 'true')

      for (var i = 0; i < categoryRadio.length; i++) {
        categoryRadio[i].addEventListener('click', () => {
          DLG.chgChart()
        })
      }
      ...
      if (datas.length) {
        if (isNew) {
          if (isCombi) {
            ...
            for (var i = 0; i < datas.length; i++) {
              if (i < half) {
                DLG.Tag.querySelector('#' + datas[i]['col'] + '_____1').setAttribute(
                  'checked',
                  'true',
                )
                datas[i]['type'] = cls1.Value
              } else {
                DLG.Tag.querySelector('#' + datas[i]['col'] + '_____2').setAttribute(
                  'checked',
                  'true',
                )
          ...
          if (isCombi) {
            if (selectedCols[0].indexOf('_____1') > 0 || selectedCols[0].indexOf('_____2') > 0) {
              ...
              for (var i = 0; i < selectedCols.length; i++) {
                if (i < half) {
                  DLG.Tag.querySelector('#' + selectedCols[i] + '_____1').setAttribute(
                    'checked',
                    'true',
                  )
                } else {
                  DLG.Tag.querySelector('#' + selectedCols[i] + '_____2').setAttribute(
                    'checked',
                    'true',
                  )
                }
            ...
            if (selectedCols[0].indexOf('_____1') > 0 || selectedCols[0].indexOf('_____2') > 0)
              selectedCols = selectedCols.map((s) => s.substring(0, s.length - 6))
            for (var i = 0; i < selectedCols.length; i++) {
              DLG.Tag.querySelector('#' + selectedCols[i]).setAttribute('checked', 'true')
            }
          }
        ...
        if (isCombi) {
          DLG.Tag.querySelector('#checkAll_____1').addEventListener('click', () => {
            DLG.chgChart()
          })

          DLG.Tag.querySelector('#checkAll_____2').addEventListener('click', () => {
            DLG.chgChart()
          })
        }
        ...
        for (var c = 0; c < cols.length; c++) {
          ...
          if (type == 'Int' || type == 'Float') {
            if (isCombi) {
              DLG.Tag.querySelector('#' + cols[c] + '_____1').addEventListener('click', () => {
                DLG.chgChart()
              })
              DLG.Tag.querySelector('#' + cols[c] + '_____2').addEventListener('click', () => {
                DLG.chgChart()
              })
            } else {
              DLG.Tag.querySelector('#' + cols[c]).addEventListener('click', () => {
                DLG.chgChart()
              })
            }
          }
    ...
    DLG.chgSize = (w, h) => {
      ...
      if (typeof h == 'string' || typeof h == 'number') {
        ...
        DLG.Tag.querySelector('.' + classPrefix + 'ChartShow').style.height =
          DLG.Tag.firstChild.getBoundingClientRect().height - 58 + 'px'
      }
    }
    DLG.downBtnToggle = () => {
    ...
    DLG.toggleConfigShow = function (init) {
      ...
      var btn = !init
        ? event.target
        : doc.querySelector('.' + classPrefix + 'ChartExtraBtn .Config')
      if (
        !doc.querySelector('.' + classPrefix + 'ChartConfig').style.width ||
        doc.querySelector('.' + classPrefix + 'ChartConfig').style.width === '0px'
      ) {
    ...
    DLG.chgChart = function () {
      ...
      if (
        ...
        doc.querySelector('#_none') &&
        doc.querySelector('#_none').checked
      ) {
        ...
        if (event.target.id == 'useDataLabel' || event.target.id.indexOf('legendAlign') != -1)
          this.sheet.showMessageTime(
            this.sheet.Lang.Dialog
              ? this.sheet.Lang.Dialog.ErrMsg.InvalidSetting
              : 'InvalidSetting',
          )
      }
      var options = {
      ...
      if (
        DLG.datas.length === 0 ||
        (doc.querySelector('#pivotData').checked && DLG.datas[0].data.length === 0)
      ) {
        this.sheet.showMessageTime(
          this.sheet.Lang.Dialog ? this.sheet.Lang.Dialog.ErrMsg.AtLeastOneData : 'AtLeastOneData',
        )
    ...
    function getCategory() {
      ...
      if (doc.querySelector('#pivotData').checked) {
        ...
        var lastHeaderRow = this.getHeaderRows()[this.getHeaderRows().length - 1]
        var selectCols = Array.prototype.slice
          .call(DLG.Tag.querySelectorAll('.datas input:checked'))
          .map((input) => input.id)
        var chartType = DLG.Tag.querySelector('#tab1Con input[type=radio]:checked').id
        if (chartType.indexOf('combination') === 0) {
          selectCols = selectCols.map((c) => {
        ...
        categories = (isAll ? this.getDataRows(1, 1) : this.getSelectedRows(0, 'Visible'))
          .filter((r) => r.Name != 'SubSum')
          .map(function (r) {
            return this.getString(r, categoryCol)
          }, this)
    ...
    function getDatas() {
      ...
      var isAll = DLG.Tag.querySelector('#_allData').checked
      var selectCols,
        selectRows = (isAll ? this.getDataRows(1, 1) : this.getSelectedRows(0, 'Visible')).filter(
          ((r) => r.Name !== 'SubSum').bind(this),
        ),
        datas = []
      var doc = this.Document != document ? this.Document : document
      ...
      if (doc.querySelector('#pivotData').checked) {
        if (chartType.indexOf('combination') === 0) {
          ...
          selectCols = Array.prototype.slice
            .call(DLG.Tag.querySelectorAll('#tab2Con .datas ul input:checked:not(#_allData)'))
            .map((input) => input.id)
          var categoryCol = DLG.Tag.querySelector(
            '#tab2Con .categories input[type=radio]:checked',
          ).id
          for (var r = 0; r < selectRows.length; r++) {
            var series = {}
        ...
        if (chartType.indexOf('combination') === 0) {
          selectCols = Array.prototype.slice
            .call(DLG.Tag.querySelectorAll('#tab2Con .datas ul input:checked'))
            .map((input) => input.id)
  ...
  Fn.showConfigDialog = function (width, height, headerIndex, name) {
    ...
    this.Document != document && this.ControlsTag
      ? this.Document.appendChild(styles)
      : document.body.appendChild(styles)
    ...
    var ConfigSheet = _IBSheet.create(
      name,
      tmpSheetTag,
      {
        Cfg: {
        ...
        Cols: [
          {
            Header: this.Lang.Dialog.HeaderTitle,
            Type: 'Text',
            Name: 'HTitle',
            RelWidth: 3,
            Hint: 1,
    ...
    dialogOpt.Foot =
      ...
      name +
      "_OkConfigDialog'>" +
      this.Lang.MenuButtons.Ok +
      '</button>' +
      "  <button id='" +
      name +
      "_CancelConfigDialog'>" +
      this.Lang.MenuButtons.Cancel +
      '</button>' +
      '</div>'
    dialogOpt.Body =
      "<div id='" +
      name +
      "_ConfigDialogBody' style='width:" +
      width +
      'px;height:' +
      height +
    ...
    for (var elem = tmpSheetTag.firstChild; elem; elem = tmpSheetTag.firstChild)
      ConfigDlgBody.appendChild(elem)
    ...
    var myArea = ConfigSheet.getElementsByClassName(
      dialogOpt.Tag,
      themePrefix + classDlg + 'HeadText',
    )[0]
    ...
    var myArea2 = ConfigSheet.getElementsByClassName(
      dialogOpt.Tag,
      themePrefix + classDlg + 'Foot',
    )[0]
    myArea2.onclick = () => {
      if (this.ARow == null) {
        ConfigSheet.blur()
    ...
    btnOk.onclick = () => {
      ...
      for (var r = 0; r < rows.length; r++) {
        ...
        if (rows[r]['Show']) {
          delete this.Cols[rows[r]['ColName']]['UserHidden']
        } else {
          this.Cols[rows[r]['ColName']]['UserHidden'] = true
        }
      }
      this.saveCurrentInfo() //현재상태 저장
      this.rerender() //화면 렌더링
  ...
  /* 멀티레코드 코드 정리해주는 코드 */
  Fn.collectColsByMultiRecord = (cols) => {
    var tempCols = []
  ...
  Fn.initImportSheet = function (
    ...
    if (fullLoadData) {
      var sheetData = fullLoadData['sheetData']
      var fileName = fullLoadData['fileName']
    }
    ...
    sheetOpts.Def = {}
    sheetOpts.Def.Row = {}
    sheetOpts.Def.HeadRowDef = {}
    var calcName = ''
    var seqIndex = -1
    var sheetColslength = 0

    if ((headType == undefined || headType == null) && this.ExcelUploadDialogHeadType) {
      headType = this.ExcelUploadDialogHeadType
    }
    var headCanEdit = headType == 'drag' ? 0 : 1
    ...
    if (!sheetOpts.Cfg['CanDrag'] || !sheetOpts.Cfg['DragCell']) {
      sheetOpts.Cfg.CanDrag = 1
      sheetOpts.Cfg.DragCell = 1
    }
    ...
    for (var i = 0; i < sheetColslength; i++) {
      if (sheetOpts.Cols[i]) {
        sheetOpts.Cols[i].Visible = !sheetOpts.Cols[i].Visible
          ? sheetOpts.Cols[i].Visible == null
            ? 1
            : 0
          : 1
      ...
      if (sheetOpts.Cols[i].Visible) {
        for (var j = 0; j < sheetHeaderRows.length; j++) {
          headerEnumString = headerEnumString.concat(
            (j > 0 ? '/' : '') +
              (sheetOpts.Cols[i].Required == 1
                ? sheetHeaderRows[j][sheetOpts.Cols[i].Header.Value] +
                  '(' +
                  this.Lang.Dialog.Required +
                  ')'
                : sheetHeaderRows[j][sheetOpts.Cols[i].Header.Value]),
          )
        }
    ...
    sheetOpts.Def.Row = { CanFormula: 1, CalcOrder: calcName, CanDrag: 0 }

    if (headType == 'drag') {
      sheetOpts.Def.HeadRowDef = { CanDrag: 1 }
    } else if (headType == 'select') {
      sheetOpts.Def.HeadRowDef = { CanDrag: 0, CanEdit: 1 }
    }

    for (var i = 0; i < sheetOpts.Cols.length; i++) {
      if (
        sheetOpts.Cols[i] &&
        (typeof sheetOpts.Cols[i].Name == 'undefined' ||
          sheetOpts.Cols[i].Name == '' ||
          sheetOpts.Cols[i].Name == null)
      ) {
      ...
      sheetOpts.Cols[i].ColorFormula = (fr) => {
        ...
        if (type === 'Date') {
          var maskedDate = _IBSheet.stringToDate(
            '' + fr.Value,
            fr.Sheet.Sheet.Cols[fr.HeadRow[fr.Col]].DataFormat,
          )
          if (isRequired) {
            if (fr.Value == null || (_IBSheet.__.isString(fr.Value) && fr.Value == '')) {
              return colorError
      ...
      sheetOpts.Cols[i].Header = ((arr, idx, sheet) => {
        arr.push({
    ...
    var defaultEvents = {
      onRenderFirstFinish: (evtParams) => {
        var sheetCols = evtParams.sheet.getCols()
        if (evtParams.sheet.RowIndex && evtParams.sheet.RowIndex != 'SEQ')
          sheetCols = sheetCols.filter((item) => item !== 'SEQ')
        var colsMap = ''
        ...
        if (uploadType === 'excel') {
          ...
          evtParams.sheet.loadText({
            mode: 'NoHeader',
            colSeparator: ' !  @  ',
            fileExt: uploadType === 'csv' ? 'csv' : '',
          })
        }
      },
      onImportFinish: (evtParams) => {
        ...
        if (uploadType === 'excel') {
          var mainSheetColCnt = evtParams.sheet.Sheet.getCols({ seq: 1 }).length

          if (dataLength < mainSheetColCnt) {
            for (var i = mainSheetColCnt; i < sheetOpts.Cols.length; i++) {
        ...
        evtParams.sheet.Sheet.makeUploadDialog(
          width,
          height,
          name,
          evtParams.sheet,
        ...
        for (var i = 0; i < sheetCols.length; i++) {
          ...
          evtParams.sheet.Cols[sheetCols[i]].MasterType =
        ...
        if (uploadType === 'txt' || uploadType === 'csv') {
          ...
          if (
            evtParams.data.length &&
            evtParams.data[0][sheetCols[startCol]].indexOf(selectOptsValue) > -1
          ) {
            var tmpRow = {}
            if (!evtParams.sheet.OrgLoadTextDataValue) evtParams.sheet.OrgLoadTextDataValue = []
            for (var i = 0; i < evtParams.data.length; i++) {
              tmpRow = {}
              var sepContents = evtParams.sheet.OrgLoadTextDataValue[i]
                ? evtParams.sheet.OrgLoadTextDataValue[i].split(selectOptsValue)
                : evtParams.data[i][sheetCols[startCol]] &&
                  evtParams.data[i][sheetCols[startCol]].split(selectOptsValue)
              if (!evtParams.sheet.OrgLoadTextDataValue[i])
                evtParams.sheet.OrgLoadTextDataValue[i] = evtParams.data[i][sheetCols[startCol]]
    ...
    var fullLoadEvents = {
      ...
      onBeforeDataLoad: (evtParams) => {
        ...
        if (uploadType === 'excel') {
          var mainSheetColCnt = evtParams.sheet.Sheet.getCols({ seq: 1 }).length
          dataLength = dataLength < mainSheetColCnt ? mainSheetColCnt : dataLength
          for (var i = 1; i < sheetOpts.Cols.length; i++) {
            if (dataLength <= i && !evtParams.data[0][sheetOpts.Cols[i].Name])
              evtParams.sheet.hideCol('EXTRACOL' + i)
    ...
    uploadDialogEvents.onBeforeChange = (evtParams) => {
      if (evtParams.row && evtParams.row.id === 'HeadRow') {
        ...
        for (var i = 2; i < sheetCols.length; i++) {
          if (
            evtParams.val &&
            evtParams.row[sheetCols[i]] == evtParams.val &&
            sheetCols[i] != evtParams.col
          ) {
          ...
          if (evtParams.row[sheetCols[i]] == evtParams.oldval) {
            for (var j = 0; j < listConvert.length; j++) {
              evtParams.sheet.Cols[evtParams.col][listConvert[j]['Name']] = !evtParams.val
                ? ''
                : evtParams.sheet.Sheet.Cols[evtParams.val][listConvert[j]['OrgName']]
              if (evtParams.col !== sheetCols[i])
                evtParams.sheet.Cols[sheetCols[i]][listConvert[j]['Name']] = !evtParams.oldval
                  ? ''
                  : evtParams.sheet.Sheet.Cols[evtParams.oldval][listConvert[j]['OrgName']]
            }
          }
        }

        if (parentSheetCols[evtParams.val] && parentSheetCols[evtParams.val].Type == 'Date')
          evtParams.sheet.Cols[evtParams.col].MasterType = 'Date'

        evtParams.sheet.refreshRow(evtParams.row)
      }
    }

    uploadDialogEvents.onAfterChange = (evtParams) => {
    ...
    if (headType == 'drag') {
      var dragCellRow
      uploadDialogEvents.onStartDragCell = (evtParams) => {
        if (evtParams.row.Kind == 'Head') {
          dragCellRow = evtParams.row
          if (evtParams.col != 'chkBool' && evtParams.col != 'SEQ') return
        }
      }
      uploadDialogEvents.onMouseOver = (evtParams) => {
        if (dragCellRow) {
          evtParams.sheet.setAttribute(evtParams.orow, evtParams.ocol, 'HtmlPrefix', '')
          if (
            evtParams.row &&
            evtParams.event.buttons == 1 &&
            evtParams.row.Kind == 'Head' &&
            evtParams.col != 'chkBool' &&
            evtParams.col != 'SEQ'
          ) {
            evtParams.sheet.setAttribute(
              evtParams.row,
              evtParams.col,
              'HtmlPrefix',
              "<span style='background-repeat:no-repeat!important;width:14px;height:14px;background-image:url(data:image/svg+xml;base64,);'></span>",
            )
          }
        }
      }

      uploadDialogEvents.onEndDragCell = (evtParams) => {
        if (
          evtParams.row &&
          evtParams.torow &&
          evtParams.row.id === 'HeadRow' &&
          evtParams.torow.id === 'HeadRow'
        ) {
          evtParams.sheet.setAttribute(evtParams.torow, evtParams.tocol, 'HtmlPrefix', '', 0)
          var val = evtParams.torow[evtParams.tocol]
          var oldval = evtParams.row[evtParams.col]
          ...
          for (var i = 2; i < sheetCols.length; i++) {
            if (val && evtParams.torow[sheetCols[i]] == val && sheetCols[i] != evtParams.col) {
              evtParams.torow[sheetCols[i]] = oldval
            }
            if (evtParams.torow[sheetCols[i]] == oldval) {
              for (var j = 0; j < listConvert.length; j++) {
                evtParams.sheet.Cols[evtParams.tocol][listConvert[j]['Name']] = !val
                  ? ''
                  : evtParams.sheet.Sheet.Cols[val][listConvert[j]['OrgName']]
                if (evtParams.tocol !== sheetCols[i])
                  evtParams.sheet.Cols[sheetCols[i]][listConvert[j]['Name']] = !oldval
                    ? ''
                    : evtParams.sheet.Sheet.Cols[oldval][listConvert[j]['OrgName']]
              }
            }
          }
          if (parentSheetCols[val] && parentSheetCols[val].Type == 'Date')
            evtParams.sheet.Cols[evtParams.tocol].MasterType = 'Date'

          evtParams.row[evtParams.col] = val

          evtParams.sheet.refreshRow(evtParams.torow)
          evtParams.sheet.calculate(true)
        }
      }
    }
  ...
  Fn.makeUploadDialog = function (
    ...
    styles.textContent =
      ...
      themePrefix +
      classDlg +
      'Title > div:last-child > div:first-child > label {' +
    ...
    if (sheetData) {
      ...
      for (var i = 0; i < sheetData.length; i++) {
        var fullLoadSheetOpt =
          "<option value='" + i + "'>" + sheetData[i]['sheetName'] + '</option>'
    ...
    if (uploadType === 'excel') {
      ...
      if (sheetData && fullLoadSheetOptArr) {
        dialogOpt.Body +=
          '  <div>' +
          "    <ul class='' style='list-style:none;padding:0;margin:0 0 5px 0;'>" +
          '      <li>' +
          "        <div style='display:flex;align-items:center;height:30px;'>" +
          "          <label for='" +
          name +
          "_UploadFileName' style='font-size:15px;width:80px;'>" +
          this.Lang.Dialog.FileName +
          '          </label>' +
          "          <div id='excelUploadSheet_sheet_UploadFileName' name='_UploadFileName' style='flex:1;border:1px solid #cdcdcd;padding:0 0 0 6px;font-size:13px;height:22px;line-height:22px;text-align:left;'>" +
          fileName +
          '          </div>' +
          '        </div>' +
          '      </li>' +
          ...
          "          <label for='" +
          name +
          "_UploadSheetName' style='font-size:15px;width:80px;'>" +
          this.Lang.Dialog.SheetName +
          '          </label>' +
          "          <select id='excelUploadSheet_sheet_UploadSheetName' name='_UploadSheetName' style='flex:1;font-size:13px;height:24px;'>" +
          fullLoadSheetOptArr +
          '          </select>' +
          '        </div>' +
          '      </li>' +
      ...
      dialogOpt.Body +=
        ...
        this.Lang.Dialog.DataPreview +
        '</label></div>' +
        "    <div style='float:right;'><button type='button' id='add_Btn' class='" +
        name +
        "_Btn'>" +
        this.Lang.Dialog.AddRow +
        "</button><button type='button' id='delete_Btn' class='" +
        name +
        "_Btn'>" +
        this.Lang.Dialog.DelRow +
        "</button><button type='button' id='rule_Btn' class='" +
        name +
        "_Btn'>" +
        this.Lang.Dialog.ShowDetail +
        '</button></div>' +
        '  </div>' +
        '</div>' +
        "<div style='width:" +
        ...
        "dialogRule'>" +
        '   <p>' +
        this.Lang.Dialog.Caution1 +
        '</p>' +
        '   <p>' +
        this.Lang.Dialog.Caution2 +
        '</p>' +
        '   <p>' +
        this.Lang.Dialog.Caution3 +
        '</p>' +
        '   <p>' +
        this.Lang.Dialog.Caution4 +
        '</p>' +
        '   <p>' +
        this.Lang.Dialog.Caution5 +
        '</p>' +
        '</div>' +
        "<div id='" +
        name +
        "_ExcelUpLoadPopupBody' style='width:" +
        width +
        'px;height:' +
        height +
        "px;overflow:hidden;'></div>"
    ...
    if (uploadType === 'txt' || uploadType === 'csv') {
      dialogOpt.Body =
        ...
        "          <label for='" +
        name +
        "_UploadTextSep' style='font-size:15px;font-weight:bold;'>" +
        this.Lang.Dialog.SetDelimiter +
        '</label>' +
        "          <select id='" +
        name +
        "_UploadTextSep' style='margin-left: 3px;font-size:15px;height:25px;'>" +
        "            <option value=','>,</option>" +
        ...
        '          </select>' +
        "          <span style ='float:right;padding-right:109px;'><button id='textSepApply'>" +
        this.Lang.MenuButtons.Apply +
        '</button></span>' +
        '      </li>' +
        '    </ul>' +
        '  </div>' +
    ...
    if (GetElem('textSepApply')) {
      ...
      textSepApply.onclick = () => {
        ...
        for (var i = 0; i < dataRows.length; i++) {
          tmpRow = {}
          var sepContents = uploadSheet.OrgLoadTextDataValue[i]
            ? uploadSheet.OrgLoadTextDataValue[i].split(selectOptsValue)
            : dataRows[i][sheetCols[startCol + 1]] &&
              dataRows[i][sheetCols[startCol + 1]].split(selectOptsValue)
          if (!uploadSheet.OrgLoadTextDataValue[i])
            uploadSheet.OrgLoadTextDataValue[i] = dataRows[i][sheetCols[startCol + 1]]
        ...
        for (var j = 0; j < cols.length; j++) {
          ...
          if (cols[j] != 'SEQ' && cols[j] != 'chkBool' && chkRows[i][cols[j]] != null) {
            if (!hr[cols[j]]) data.data[i][hr[cols[j]]] = ''
              ...
              if (
                uploadSheet.Cols[cols[j]].MasterType == 'Date' &&
                typeof chkRows[i][cols[j]] == 'number'
              ) {
                ...
                var uploadDataFormat =
                  uploadSheet.ParentSheet.Cols[hr[cols[j]]].DataFormat ||
                  uploadSheet.ParentSheet.Cols[hr[cols[j]]].Format ||
                  'yyyy/mm/dd'
    ...
    btnRule.onclick = () => {
      ...
      if (dialogRule.style.display === 'none') {
        dialogRule.style.display = 'inline-block'
      } else {
  ...
  Fn.parseFullLoadData = function (data, opt) {
    var replaceEntities = (v) => {
      if (v.indexOf('&#') >= 0) {
        var A = v.match(/&#x\w*;/gi)
        if (A)
          for (var i = 0; i < A.length; i++)
            v = v.replace(A[i], String.fromCharCode(parseInt(A[i].slice(3), 16)))
        var A = v.match(/&#\w*;/gi)
        if (A)
          for (var i = 0; i < A.length; i++)
            v = v.replace(A[i], String.fromCharCode(parseInt(A[i].slice(2), 10)))
      }
      return v
        .replace(/&lt;/g, '<')
        .replace(/&gt;/g, '>')
      ...
      try {
        ...
        if (data) {
          ...
          for (var j = 0; j < data['sheetData'].length; j++) {
            ...
            else if (data['sheetData'][j].maxCol > data['sheetData'][j - 1].maxCol)
              colCount = data['sheetData'][j].maxCol
          }
          if (colCount > this.getCols()) colCount = colCount - this.getCols().length

          this.initImportSheet(
            data['fileType'],
            Math.abs(700),
            Math.abs(500),
            name,
  ...
  Fn.showExcelDownloadDialog = function (width, height, name, rowchk, title, downParams) {
    ...
    if (rowchk) {
      ...
      if (downParams && downParams.downRows) {
        ...
        var chkrows = '|'
        for (var i = 0, j = this.getDataRows(); i < j.length; i++) {
          chkrows = chkrows + i + '|'
        }
      }
      ...
      opts.Events = {
        onDataLoad: (evtParams) => {
          for (
            var i = 0, j = this.getDataRows(), k = evtParams.sheet.getDataRows();
            i < j.length;
            i++
          ) {
    ...
    var storageVal = localStorage.getItem(name + 'DownLoadSheetInfo')
    var storageChk = false
    if (storageVal && storageVal !== '{}') {
      var parsed = JSON.parse(storageVal)
      var chkcols = '|' + parsed.Cols.join('|') + '|'
      storageChk = true
    } else if (
      !downParams ||
      typeof downParams.donwCols == 'undefined' ||
      (downParams && downParams.downCols == 'Visible')
    ) {
      var chkcols = '|'
      for (var s = 0; s < sec.length; s++) {
        if (opts[sec[s]]) {
          for (var i = 0; i < opts[sec[s]].length; i++) {
            if (sec[s] == 'LeftCols' && opts[sec[s]][i] && opts[sec[s]][i].Name == 'chkBool')
              continue
    ...
    for (var s = 0; s < sec.length; s++) {
      if (opts[sec[s]]) {
        opts[sec[s]].forEach((col) => {
          ...
          if (downParams && downParams.downCols == 'Visible' && C[col.Name] && !C[col.Name].Visible)
            col.Visible = 0
          opts.Head[opts.Head.length - 1][col.Name] = {
            Type: 'Bool',
            Value:
              chkcols.indexOf('|' + col.Name + '|') != -1 && C[col.Name] && C[col.Name].CanExport
                ? 1
                : 0,
    ...
    if (opts && opts.Cfg && opts.Cfg.StyleRowConfig) {
      delete opts.Cfg.StyleRowConfig
      opts.Cfg.Style = this.Style
    }

    var DownSheet = _IBSheet.create(name, tmpSheetTag, opts, this.deepCopyRows())
    /* step 4 end */
    ...
    dialogOpt.Foot =
      ...
      "  <button id='" +
      name +
      "_ExcuteExcelDownLoad'>" +
      this.Lang.Dialog.Download +
      '</button>' +
      "  <button id='" +
      name +
      "_CancelExcelDownLoad'>" +
      this.Lang.MenuButtons.Cancel +
      '</button>' +
      '</div>'
    dialogOpt.Body =
      ...
      "      <label for='" +
      name +
      "_DownloadSelectSave' style='display:inline-block'>" +
      this.Lang.Dialog.DownloadColSave +
      '</label>' +
      "      <input type='checkbox' id='" +
      name +
      "_DownloadSelectSave' name='DownloadSelectSave' style='display:inline-block'" +
      (storageChk ? ' checked' : '') +
      '>' +
      '    </div>' +
      '  </div>' +
      ...
      "<div id='" +
      name +
      "_ExcelDownPopupBody' style='width:" +
      width +
      'px;height:' +
      height +
      "px;overflow:hidden;'></div>" +
      ...
      "          <input type='radio' id='" +
      name +
      "_DownloadExcel' name='DownloadType' value='1' checked='checked' style='margin-left:0px;'>" +
      "          <label for='" +
      name +
      "_DownloadExcel'>" +
      this.Lang.Dialog.XlsxDown +
      ...
      "          <input type='radio' id='" +
      name +
      "_DownloadText' name='DownloadType' value='2'>" +
      "          <label for='" +
      name +
      "_DownloadText'>" +
      this.Lang.Dialog.TextDown +
      '</label>' +
      '        </span>' +
      ...
      "          <label for='" +
      name +
      "_DownloadFileName' style='display:inherit;font-size:15px;font-family:'NotoSans_Bold''>" +
      this.Lang.Dialog.FileName +
      '</label>' +
      "          <input type='text' id='" +
      name +
      "_DownloadFileName' name='DownloadFileName' style='display:inherit;margin-left: 3px;font-size:15px;width:75%;text-align:left;height:25px' value='" +
      fileName +
      "'>" +
      '        </span>' +
      '      </li>' +
    ...
    for (var elem = tmpSheetTag.firstChild; elem; elem = tmpSheetTag.firstChild)
      ExcelDownDlgBody.appendChild(elem)
    ...
    var myArea = DownSheet.getElementsByClassName(
      dialogOpt.Tag,
      themePrefix + classDlg + 'HeadText',
    )[0]
    ...
    var myArea2 = DownSheet.getElementsByClassName(
      dialogOpt.Tag,
      themePrefix + classDlg + 'Foot',
    )[0]
    myArea2.onclick = () => {
    ...
    btnDownload.onclick = () => {
      var fileName =
        GetElem(name + '_DownloadFileName') && GetElem(name + '_DownloadFileName').value
          ? GetElem(name + '_DownloadFileName').value
          : 'sheet'
      var checkHeader = DownSheet.getRowById('downCheckHeader')
      var cols = DownSheet.getCols()
      var str = cols.filter((col) =>
        col === 'SEQ'
          ? DownSheet.Cols[col].Visible
          : !(checkHeader[col + 'Visible'] == 0 || checkHeader[col + 'Type'] != 'Bool') &&
            checkHeader[col] === 1 &&
            DownSheet.getAttribute(null, col, 'Visible'),
      )
      var rows = []
      var rowStr = ''
      var saveStorage = GetElem(name + '_DownloadSelectSave')

      if (saveStorage.checked) {
        localStorage.setItem(name + 'DownLoadSheetInfo', JSON.stringify({ Cols: str }))
      } else {
        localStorage.setItem(name + 'DownLoadSheetInfo', '{}')
      }
      ...
      if (GetElem(name + '_DownloadExcel').checked) {
        try {
          ...
          this.down2Excel(paramObj)
        } catch (e) {
          if (e.message.indexOf('down2Excel is not a function') > -1) {
            console.log(
              '%c ' + this.Lang.Dialog.Caution,
              'color:#FF0000',
              ' : ' + this.Lang.Dialog.NeedCommonFile,
            )
          }
        }
      } else {
...
*/
  Fn.showUploadDialog = function (uploadType, width, height, name, colCount, fullLoad, headType) {
    var minWidth = 700
    var minHeight = 400
    var count = 20
    ...
    if (typeof uploadType == 'object') {
      ...
      if (uploadTemp.fullLoad != null) fullLoad = uploadTemp.fullLoad
      if (uploadTemp.headType != null) headType = uploadTemp.headType
    }
    if (typeof headType == 'string') headType = headType.toLowerCase()
    if (fullLoad) {
      IBSheet[this.id].ExcelUploadDialogHeadType = headType
      IBSheet[this.id].loadExcel({ mode: 'FullLoad' })
      return
    }
    ...
    if (headType == null || headType == undefined) headType = 'select'
  ...
  Fn.getRowEditData = function (row, headerIndex, excludeHideCol, nav) {
    ...
    if (Array.prototype.filter) {
      cols = this.getCols().filter((col) => col != 'SEQ')
    } else {
      cols = []
    ...
    if (headerIndex != null && typeof headerIndex === 'number' && headerIndex >= 0) {
      ...
      for (var j = 0; j < cols.length; j++) {
        ...
        for (var i = 0; i < headerRows.length; i++) {
          if (
            headerRows[i][cols[j]] != null &&
            headerRows[i][cols[j]] !== '' &&
            headerRows[i][cols[j] + 'RowSpan'] !== 0
          ) {
  ...
  Fn.makeEditHtml = function (row, headerIndex, excludeHideCol) {
    ...
    html[html.length] =
      "<div style='width:80%;border-top:2px solid #000000;word-wrap:break-word;'><ul>"

    for (var i = 0; i < cols.length; i++) {
      if (excludeHideCol && !this.getAttribute(null, cols[i], 'Visible')) continue
      // 컬럼명
      s =
        "<li style='margin:10px 0'><label style='display:inline-block;width:200px' for='htmlEdit_" +
        cols[i] +
        "'>" +
        (headerIndex != null ? header[headerIndex][cols[i]] : header[i]) +
        '</label>'
      ...
      switch (this.getType(row, cols[i])) {
        case 'Lines':
          s +=
            "<textarea style='width:100%;height:80px' id='htmlEdit_" +
            cols[i] +
            "' title='" +
            this.Cols[cols[i]].Header +
            "' />" +
            val +
            '</textarea>'
          break
        ...
        case 'Float':
          s +=
            "<input style='width:100%;height:30px' type='text' value='" +
            val +
            "' id='htmlEdit_" +
            cols[i] +
            "' title='" +
    ...
    function getRadioString(row, col) {
      ...
      if (enumVal.length) {
        for (var idx = 0; idx < enumVal.length; idx++) {
          ...
          result +=
            "<input type='radio' name='htmlEdit_" +
            col +
            "' value='" +
            tmp +
            "'" +
  ...
  Fn.showEditDialog = function (row, width, height, headerIndex, name, excludeHideCol, nav) {
    ...
    dialogOpt.Body =
      ...
      (nav
        ? " <div><div id='" +
          ...
          classDlg +
          'NavBtns ' +
          (row && !this.getPrevVisibleRow(row) ? themePrefix + classDlg + 'NavBtnsDisable' : '') +
          "'><span>◀</span></div></div>"
        : '') + // 왼쪽으로 이동 버튼
      " <div id='" +
      name +
      "_EditSheetDiv' style='display:flex;'>" +
      "   <div id='" +
      name +
      "_EditDialogBody' style='width:" +
      width +
      'px;height:' +
      height +
      "px;overflow:hidden;'></div>" +
      (nav
        ? "   <div id='" +
          name +
          "_EditNavDialogBody' style='width:" +
          width +
          'px;height:' +
          height +
          "px;display:none;' class='" +
          themePrefix +
          classDlg +
          "NavBody'></div>"
        : '') + // 숨겨진 시트
      (nav
        ? "   <div id='" +
          ...
          themePrefix +
          classDlg +
          "NavMovedBody'><span>▶</span></div>"
        : '') +
      (nav
        ? "   <div id='" +
          name +
          "_MovedNextBody' style='width:" +
          width +
          'px;height:' +
          height +
          "px;display: none;' class='" +
      ...
      (nav
        ? " <div><div id='" +
          ...
          classDlg +
          'NavBtns ' +
          (row && !this.getNextVisibleRow(row) ? themePrefix + classDlg + 'NavBtnsDisable' : '') +
          "'><span>▶</span></div></div>"
        : '') + // 오른쪽으로 이동 버튼
      '</div>'
    ...
    if (nav) {
      ...
      for (var elem = tmpNavSheetTag.firstChild; elem; elem = tmpNavSheetTag.firstChild)
        EditNavDlgBody.appendChild(elem)
    ...
    var myArea = EditSheet.getElementsByClassName(
      dialogOpt.Tag,
      themePrefix + classDlg + 'HeadText',
    )[0]
    var myArea2 = EditSheet.getElementsByClassName(
      dialogOpt.Tag,
      themePrefix + classDlg + 'Foot',
    )[0]
    ...
    btnOk.onclick = () => {
      ...
      if (EditSheet.getRowsByStatus('Changed').length) {
        ...
        while (prow) {
          this.setValue(currentRow, prow['ColName'], prow['Target'], 1)
    ...
    if (nav && btnNavPrev && btnNavNext) {
      btnNavPrev.onclick = () => {
        if (
          EditSheet.getRowsByStatus('Changed').length &&
          confirm(EditSheet.Lang.Dialog.WillSave)
        ) {
          // 변경된 내용 저장
          EditSheet.endEdit(1)
          var prow = EditSheet.getFirstRow()
        ...
        // 이전 행으로 데이터 변경
        var prevRow = this.getPrevVisibleRow(currentRow)
        if (!prevRow) return
        currentRow = this.updateCurrentRow(
          EditSheet,
          name,
          classDlg,
          prevRow,
      ...
      btnNavNext.onclick = () => {
        ...
        // 이후 행으로 데이터 변경
        var nextRow = this.getNextVisibleRow(currentRow)
        if (!nextRow) return
        currentRow = this.updateCurrentRow(
          EditSheet,
          name,
    ...
    if (nav) {
      ...
      if (!dialogOuter && this.Document)
        this.Document.querySelectorAll(themePrefix + classDlg + 'Outer')[0]
      var outer = getComputedStyle(
        document.getElementsByClassName(themePrefix + classDlg + 'Outer')[0],
      )
      ...
      EditSheetDiv.ontouchstart = (ev) => {
        onStart(ev)
      }

      EditSheetDiv.ontouchmove = (ev) => {
  ...
  Fn.showHtmlEditDialog = function (row, width, height, headerIndex, name, excludeHideCol) {
    ...
    dialogOpt.Body =
      ...
      " <div id='" +
      name +
      "_EditHtmlDiv' style='display:flex;'>" +
      "   <div id='" +
      name +
      "_EditDialogBody' style='width:" +
      width +
    ...
    btnOk.onclick = () => {
      ...
      for (var c = 0; c < cols.length; c++) {
        switch (this.getType(row, cols[c])) {
          case 'Radio':
            elem = document.querySelector("input[name='htmlEdit_" + cols[c] + "']:checked")
            if (elem && this.getValue(row, cols[c]) != elem.value)
              this.setValue(row, cols[c], elem.value)
          case 'Bool':
            elem = document.getElementById('htmlEdit_' + cols[c])
            if (elem && this.getValue(row, cols[c]) != elem.checked)
              this.setValue(row, cols[c], elem.checked)
            break
          case 'Date':
            elem = document.getElementById('htmlEdit_' + cols[c])
            if (elem && this.getString(row, cols[c]) != elem.value)
              this.setString(row, cols[c], elem.value)
            break
          default:
            elem = document.getElementById('htmlEdit_' + cols[c])
            if (elem && this.getValue(row, cols[c]) != elem.value)
  ...
  Fn.updateCurrentRow = function (
    ...
    var data = this.getRowEditData(row, headerIndex, excludeHideCol, nav)
    EditSheet.loadSearchData({ data: data, sync: 1 })

    if (nav) {
      // nav buttons disable 적용
      var btnNavPrev = GetElem(name + '_NavPrevBtn')
      var btnNavNext = GetElem(name + '_NavNextBtn')

      if (row && !this.getPrevVisibleRow(row)) {
      ...
      if (row && !this.getNextVisibleRow(row)) {
  ...
  Fn.dataSearch = function (row, col, fDlgSheet, match) {
    ...
    if (!row[col] && (findingVal == colInfo.EmptyValue || findingVal == row[col + 'EmptyValue']))
      return

    var checkVal = findingVal
    if (colInfo.Header) {
      findingColHeader = colInfo.Header
      if (Array.isArray(findingColHeader))
        findingColHeader = findingColHeader[findingColHeader.length - 1]
    }
    ...
    var matchCon = match
      ? checkVal == this.SearchExpression
      : checkVal.indexOf(this.SearchExpression) > -1
  ...
  Fn.findDlgFunc = function (work, evt) {
    ...
    function afterFindingToDo(ibsheet) {
      //검색 된 행/건수 표시
      window[ibsheet.id + '_FindDlg'].Tag.getElementsByTagName('span')[0].innerText =
        ibsheet.SearchCount ? T.Lang.Alert.FoundResults.replace('%d', ibsheet.SearchCount) : ''
      //클릭 객체에게 다시 포커스 부여
    ...
    switch (work) {
      ...
      case 'Replace': {
        ...
        if (match) {
          if (fvalue === this.SearchExpression) {
            flag = false
            tmp = fvalue.sheet_dialog_replaceAll(
          ...
          if (
            fvalue.indexOf(this.SearchExpression) > -1 &&
            (this.Cols[fcol].CanEdit || frow[fcol + 'CanEdit'] !== 0 || includeCantEdit)
          ) {
            flag = false
            tmp = fvalue.sheet_dialog_replaceAll(
              this.SearchExpression,
      ...
      case 'ReplaceAll': {
        ...
        while (r) {
          for (var i = 0; i < cols.length; i++) {
            ...
            if (
              (this.Cols[c].CanEdit === 0 ||
                r.CanEdit === 0 ||
                r[c + 'CanEdit'] === 0 ||
                (r.Def && r.Def.CanEdit === 0) ||
                this.CanEdit === 0) &&
              !includeCantEdit
            )
              continue
            ...
            if (match) {
              if (rc === this.SearchExpression) {
                ...
                tmp = rc.sheet_dialog_replaceAll(
                  this.SearchExpression,
                  rvalue,
                  c,
                  r,
      ...
      case 'FoldAndExpand':
        ...
        findDlgFoldBtn.style.transform =
          'rotate(' + foldExpandInfo[findDlgFoldBtn.dataset.rotate].rotate + 'deg)'
        findDlgFoldBtn.setAttribute(
          'data-rotate',
          foldExpandInfo[findDlgFoldBtn.dataset.rotate].rotate,
        )
        findDialogOuterBody.style.height = foldExpandInfo[findDlgFoldBtn.dataset.rotate].height
        // }
        break
  ...
  String.prototype.sheet_dialog_replaceAll = function (org, dest, fcol, frow, T, includeCantEdit) {
    if (
      ((T.Cols[fcol] && T.Cols[fcol].CanEdit === 0) ||
        frow.CanEdit === 0 ||
        (frow.Def && frow.Def.CanEdit === 0) ||
        frow[fcol + 'CanEdit'] === 0 ||
        T.CanEdit === 0) &&
      !includeCantEdit
    )
  ...
  Fn.showFindDialog = function () {
    ...
    for (var i = 0; i < vcols.length; i++) {
      if (
        this.Cols[vcols[i]] &&
        (this.Cols[vcols[i]].Type == 'Pass' || this.Cols[vcols[i]].Type == 'Img')
      )
        continue
    ...
    styles.textContent =
      '.' +
      themePrefix +
      'FindDlgWrap button:hover{opacity:0.5}' +
      '.' +
      themePrefix +
      'FindDlgWrap #' +
      sheetId +
      '_findDlg_sheet_fold_btn{transition: all 0.8s}'
    ...
    var DLGBODY =
      "<div class='" +
      themePrefix +
      "FindDlgWrap' style='padding-left: 8px;'>" +
      "<div class='" +
      themePrefix +
      "FindDlgTop' style='width:94.7%;display:flex;justify-content:space-between;'>" +
      "<div style='border-radius:3px;'>" +
      "<input type='text' style='border:0px;outline:none;' id='" +
      sheetId +
      "_FindTxt' title='" +
      this.Lang.Dialog.EnterSearchTerm +
      "' placeholder='" +
      this.Lang.Dialog.EnterSearchTerm +
      ...
      btnClass +
      "' title='" +
      this.Lang.Dialog.FindPrev +
      "'>↑</button>" +
      "<button type='button' tabindex='-1' class='" +
      btnClass +
      "' title='" +
      this.Lang.Dialog.FindNext +
      "'>↓</button>" +
      "<button type='button' tabindex='-1' class='" +
      btnClass +
      "' title='" +
      this.Lang.Dialog.Replace +
      "'>＋</button>" +
      '</div>' +
      '</div>' +
      "<div class='" +
      themePrefix +
      "FindDlgTop' id='fidDlgTopLayer_2' style='display:none;important;width:94.7%;justify-content:space-between;padding-top:0px;'>" +
      "<div id='sheet_edit_input_div' style='display:none;margin-top:5px;border-radius:3px;'>" +
      "<input type='text' style='border:0px;outline:none;' id='" +
      sheetId +
      "_ReplaceTxt' title='" +
      this.Lang.Dialog.EnterReplaceWord +
      "' placeholder='" +
      this.Lang.Dialog.EnterReplaceWord +
      "' autocomplete='off'/>" +
      '</div>' +
      "<div id='replace_button_div' style='display:none;margin-top:5px;border-radius:3px;'>" +
      "<button id='replace_sheet_dialog_btn' type='button' style='visibility:hidden;background:cornflowerblue;margin-top:1px;' class='" +
      btnClass +
      "'>" +
      this.Lang.Dialog.Replace +
      '</button>' +
      "<button id='replace_all_sheet_dialog_btn' type='button' style='visibility:hidden;background:cornflowerblue;margin-top:1px;' class='" +
      btnClass +
      "'>" +
      this.Lang.Dialog.ReplaceAll +
      '</button>' +
      '</div>' +
      '</div>' +
      ...
      "<div class='" +
      themePrefix +
      "S_FIND_CASE' style='margin-bottom:10px;width:94.7%;display:flex;justify-content:space-between;height:50px;position:relative;'>" +
      '<div>' +
      "<input type='checkbox' id='" +
      sheetId +
      "_FindChk' " +
      ...
      sheetId +
      "_FindChk'>" +
      this.Lang.Dialog.CaseSensitive +
      '</label>' +
      "<input type='checkbox' id='" +
      sheetId +
      "_ContentMatchChk' " +
      ...
      sheetId +
      "_ContentMatchChk'>" +
      this.Lang.Dialog.MatchEntireContent +
      '</label>' +
      "<label for='" +
      sheetId +
      "_IncludeCantEdit' id='include_cant_edit_wrap' style='position:absolute;left:0.1%;top:45%;display:none;'><input type='checkbox' id='" +
      sheetId +
      "_IncludeCantEdit' " +
      checked2 +
      ' />' +
      this.Lang.Dialog.IncludeCanNotEdit +
      '</label>' +
      '</div>' +
      '<div>' +
      ...
      this.Lang.Text.SearchMark +
      '</button>' +
      "<button type='button' class='" +
      btnClass +
      "' style='background:darkcyan; '>" +
      this.Lang.Text.SearchSelect +
      '</button>' +
      "<button type='button' class='" +
      btnClass +
      "' style='background:darkcyan;'>" +
      this.Lang.Dialog.UnCheck +
      '</button>' +
      '</div>' +
      '</div>' +
      "<div class='" +
      themePrefix +
      "S_FIND_CASE' style='margin-bottom:10px;width:94.7%;display:flex;justify-content:space-between;'>" +
      "<label for='" +
      sheetId +
      "_SelectColChk' style='padding-left:3px;display:flex;align-items:center;'>" +
      this.Lang.Dialog.SelectColumn +
      '</label>' +
      '<select id=' +
      sheetId +
      "_SelectColChk style='margin-left:6px;font-size:13px;height:25px;width:67%;outline:none;padding: 0 3px;border:1px solid #888;border-radius:3px;padding-right:24px;cursor:pointer;appearance:auto;background:none;'>" +
      str +
      '</select>' +
      '</div>' +
      "<div class='" +
      themePrefix +
      "S_FIND_BTN' style='width:94.7%;position:relative;display:flex;justify-content:space-between;'>" +
      "<button id='" +
      sheetId +
      "_findDlg_sheet_fold_btn' data-rotate='0' type='button' style='background:white;color:#7a8899;margin-left:0px;' class='" +
      btnClass +
      "'>▶</button>" +
      "<button type='button' tabindex='-1' class='" +
      btnClass +
      "' title='" +
      this.Lang.Dialog.FindAll +
      "'>" +
      this.Lang.Dialog.FindAll +
      '</button>' +
      '</div>' +
      ...
      "<div id='" +
      sheetId +
      "_FindDialogOuterBody' style='width:100%;height:0px;overflow:hidden;'>" +
      "<div id='" +
      sheetId +
      "_FindDialogBody' style='width:100%;height:300px;overflow:hidden;'>" +
      '</div>' +
    ...
    var dlg = {
      ...
      ZIndex: this.ZIndex ? this.ZIndex + 20 : 270,
      OnClose: () => {
        this.SearchExpression = window[dlgName].Tag.getElementsByTagName('input')[0].value
        delete this.SearchMethod

        //닫을때 스타일을 제거한다.
        if (styles && styles.parentNode) {
          styles.parentNode.removeChild(styles)
          delete this._stylesFindDialog
        }

        _IBSheet.Focused = this
        // 기존 창을 제거
        window[dlgName] = null
        if (this.fDlgSheet) this.fDlgSheet.dispose()
      },
    }

    dlg.Sheet = this
    dlg.Type = 'Sheet'
    window[dlgName] = _IBSheet.showDialog(dlg, {
      Align: 'center middle',
      Tag: this.MainTag,
    })
    ...
    if (!findDlgTopDiv.length && this.Document)
      findDlgTopDiv = this.Document.querySelectorAll('.' + themePrefix + 'FindDlgTop')

    var findDlgBottomDiv = document.getElementsByClassName(themePrefix + 'FindDlgBottom')[0]
    if (!findDlgBottomDiv && this.Document)
      findDlgBottomDiv = this.Document.querySelectorAll('.' + themePrefix + 'FindDlgBottom')[0]
    ...
    addEventListeners(btns[0], {
      onclick: (event) => {
        _IBSheet[sheetId].findDlgFunc('FindPrev', event)
      },
    })
    ...
    addEventListeners(btns[1], {
      onclick: (event) => {
        _IBSheet[sheetId].findDlgFunc('Find', event)
      },
    })
    ...
    addEventListeners(btns[2], {
      onclick: (event) => {
        _IBSheet[sheetId].findReplaceOpen(event)
      },
    })
    ...
    addEventListeners(btns[3], {
      onclick: (event) => {
        _IBSheet[sheetId].findDlgFunc('Replace', event)
      },
    })
    ...
    addEventListeners(btns[4], {
      onclick: (event) => {
        _IBSheet[sheetId].findDlgFunc('ReplaceAll', event)
      },
    })
    ...
    addEventListeners(btns[5], {
      onclick: (event) => {
        _IBSheet[sheetId].findDlgFunc('Mark', event)
      },
    })
    ...
    addEventListeners(btns[6], {
      onclick: (event) => {
        _IBSheet[sheetId].findDlgFunc('Select', event)
      },
    })
    ...
    addEventListeners(btns[7], {
      onclick: (event) => {
        _IBSheet[sheetId].findDlgFunc('Clear', event)
      },
    })
    ...
    addEventListeners(btns[8], {
      onclick: (event) => {
        _IBSheet[sheetId].findDlgFunc('FoldAndExpand', event)
      },
    })
    ...
    addEventListeners(btns[9], {
      onclick: (event) => {
        _IBSheet[sheetId].findDlgFunc('FindAll', event)
      },
    })
    ...
    addEventListeners(findDlgTopDiv[0], {
      onkeyup: (event) => {
        _IBSheet[sheetId].findDlgFunc('outKeyUp', event)
      },
    })
    ...
    addEventListeners(findTxtInput, {
      onkeyup: (event) => {
        _IBSheet[sheetId].findDlgFunc('FindKeyUp', event)
      },
    })
    ...
    addEventListeners(replaceTxtInput, {
      onkeyup: (event) => {
        _IBSheet[sheetId].findDlgFunc('ReplaceKeyUp', event)
      },
    })
    ...
    addEventListeners(findChk, {
      onchange: (event) => {
        _IBSheet[sheetId].findDlgFunc('ChgCaseSense', event)
      },
    })
    ...
    addEventListeners(contentMatchChk, {
      onchange: (event) => {
        _IBSheet[sheetId].findDlgFunc('ContentMatch', event)
      },
    })
    ...
    addEventListeners(includeCantEdit, {
      onchange: (event) => {
        _IBSheet[sheetId].findDlgFunc('IncludeCantEdit', event)
      },
    })
    ...
    for (var elem = tmpSheetTag.firstChild; elem; elem = tmpSheetTag.firstChild)
      FindDialogBody.appendChild(elem)
  ...
  Fn.makeSortSheetOpt = function (headerIndex, excludeHideCol, useOptions) {
    ...
    if (headerIndex != null && typeof headerIndex === 'number' && headerIndex >= 0) {
      ...
      for (var j = 0; j < cols.length; j++) {
        var headerString = []
        if (headerString.indexOf(headerRows[headerIndex][cols[j]]) < 0)
          headerString.push(headerRows[headerIndex][cols[j]])
      ...
      for (var j = 0; j < cols.length; j++) {
        var headerString = []
        for (var i = 0; i < headerRows.length; i++) {
          if (headerString.indexOf(headerRows[i][cols[j]]) < 0)
            headerString.push(headerRows[i][cols[j]])
        }
        header.push(headerString.join('/'))
      }
    }
    ...
    option.Events = {
      onAfterChange: (evtParam) => {
        ...
        if (evtParam.col == 'sTarget') {
          val =
            evtParam.sheet.ParentSheet.Cols[evtParam.val].NumberSort != null
              ? evtParam.sheet.ParentSheet.Cols[evtParam.val].NumberSort
              : numberType.indexOf(evtParam.sheet.ParentSheet.Cols[evtParam.val].Type) > -1
                ? 1
                : 0
          ...
          evtParam.sheet.setValue(
            evtParam.row,
            'sRawSort',
            evtParam.sheet.ParentSheet.Cols[evtParam.val].RawSort != null
              ? evtParam.sheet.ParentSheet.Cols[evtParam.val].RawSort
              : 0,
          )
          evtParam.sheet.setValue(
            evtParam.row,
            'sCaseSensitive',
            evtParam.sheet.ParentSheet.Cols[evtParam.val].CaseSensitive != null
              ? evtParam.sheet.ParentSheet.Cols[evtParam.val].CaseSensitive
              : 1,
          )
        }
      },
    ...
    if (this.Sort) {
      ...
      for (var i = 0; i < sort.length; i++) {
        ...
        obj['sOrder'] = order
        obj['sNumberSort'] =
          C[col].NumberSort != null
            ? C[col].NumberSort
            : numberType.indexOf(C[col].Type) > -1
              ? 1
              : 0
...
*/
  Fn.showSortDialog = function (width, height, headerIndex, name, excludeHideCol, useOptions) {
    ...
    dialogOpt.Body =
      ...
      name +
      "_Btn'>" +
      this.Lang.Dialog.AddCriteria +
      '</button>' +
      "      <button type='button' id='remove_Btn' class='" +
      name +
      "_Btn'>" +
      this.Lang.Dialog.DelCriteria +
      '</button>' +
      "      <button type='button' id='copy_Btn' class='" +
      name +
      "_Btn'>" +
      this.Lang.Dialog.CopyCriteria +
      '</button>' +
      "      <button type='button' id='moveup_Btn' class='" +
      name +
      "_Btn'>↑</button>" +
      "      <button type='button' id='movedown_Btn' class='" +
      name +
      "_Btn'>↓</button>" +
      '    </div>' +
      '  </div>' +
      ...
      "      <button type='button' id='clear_Btn' class='" +
      name +
      "_Btn'>" +
      this.Lang.Dialog.DelAllCriteria +
      '</button>' +
      '    </div>' +
      '  </div>' +
      '</div>' +
     ...
     * 버튼 클릭시 및 다이얼로그의 시트가 아닌 부분을 클릭했을 때
     */
    var myArea = SortSheet.getElementsByClassName(
      dialogOpt.Tag,
      themePrefix + classDlg + 'HeadText',
    )[0]
    ...
    var myArea2 = SortSheet.getElementsByClassName(
      dialogOpt.Tag,
      themePrefix + classDlg + 'Foot',
    )[0]
    myArea2.onclick = () => {
    ...
    btnOk.onclick = () => {
      ...
      while (prow) {
        ...
        if (sortInfo.indexOf((prow['sOrder'] == 'asc' ? '' : '-') + prow['sTarget']) > -1) {
          SortSheet.showMessageTime(
            SortSheet.getString(prow, 'sTarget') + SortSheet.Lang.Dialog.SortWarn2,
          )
          return
        }
        ...
        // NumberSort 설정 적용
        this.setAttribute(null, prow['sTarget'], 'NumberSort', parseInt(prow['sNumberSort']))
        this.setAttribute(null, prow['sTarget'], 'RawSort', parseInt(prow['sRawSort']))
        this.setAttribute(null, prow['sTarget'], 'CaseSensitive', parseInt(prow['sCaseSensitive']))

        prow = SortSheet.getNextRow(prow)
      }
      sortStr = sortInfo.join(',')

      sortStr ? this.doSort(sortStr) : this.clearSort()
      SortSheet.dispose()
      this.closeDialog()
      // 버튼을 닫을때 스타일을 제거한다.
      if (styles && styles.parentNode) {
    ...
    var btnAdd = GetElem('add_Btn')
    btnAdd.onclick = () => {
      if (this.MaxSort <= SortSheet.getDataRows().length) {
        SortSheet.showMessageTime(SortSheet.Lang.Dialog.SortWarn3.replace('%d', this.MaxSort))
        return
      }
    ...
    btnMovedown.onclick = () => {
      var focusRow = SortSheet.getFocusedRow(),
        targetRow = focusRow
          ? SortSheet.getNextRow(focusRow)
            ? SortSheet.getNextRow(SortSheet.getNextRow(focusRow))
            : null
          : null
  ...
  Fn.showPivotDialog = function (width, height, name, pivotParams, useStorage) {
    ...
    var Pos = {
      Align: 'center middle',
      Tag: this.PivotSheet
        ? this.PivotSheet.MainTag.style.display === '' && this.MainTag.style.display == 'none'
          ? this.PivotSheet.MainTag
          : this.MainTag
        : this.MainTag,
    }
    ...
    dialogOpt.Body =
      ...
      "px;'>" +
      '<div>' +
      "<div style='padding-bottom: 10px;'><input id='searchCol' style='width:99%; padding-inline: 2px 0;' autocomplete='off' type='text' placeholder='" +
      this.Lang.Dialog.PivotColSearch +
      "'>" +
      "<button id='" +
      name +
      "_searchCol_ClearBtn' class='" +
      themePrefix +
      'SideRight ' +
      themePrefix +
      "Filter0Right ' style='display: none;'></button>" +
      '</div>' +
      ...
      this.Lang.Dialog.NormalColumn +
      '</div>' +
      "<div id='" +
      name +
      "_NomalCols' name='NomalCols' class='" +
      this.Style +
      "PivotStandards' style='overflow: auto; height:calc(100% - 44px);padding:7px;border:1px solid #c5c5c5;'>" +
      (pivotCols[0] ? pivotCols[0] : '') +
      '</div>' +
      '</div>' +
      // 필터
      "<div style='padding-left: 3px; padding-bottom: 5px; font-size: 10px; width:" +
      width +
      "px;'>" +
      this.Lang.Dialog.PivotInfoMsg +
      '</div>' +
      "<div style='padding-left: 3px; font-size:10px; padding-bottom: 7px; width:" +
      width +
      "px;'>" +
      this.Lang.Dialog.PivotColDelete +
      '</div>' +
      "<div style='height: 30%;'>" +
      "<div style='border-bottom: 1px solid #c5c5c5;border-top: 1px solid #c5c5c5;'>" +
      this.Lang.Text.SearchFilter +
      '</div>' +
      "<div id='" +
      name +
      "_PivotFilter' class='" +
      ...
      '_PivotStandards ' +
      this.Style +
      "PivotStandards' style='overflow: auto; height:calc(100% - 46px);padding:7px;border:1px solid #c5c5c5;'>" +
      ...
      '_PivotStandards ' +
      this.Style +
      "PivotStandards' style='overflow: auto; height:calc(100% - 45px);padding:7px; border-radius: 3px;border:1px solid #c5c5c5;'>" +
      ...
      "<div class='" +
      name +
      "_table' style='float:right'>" +
      // 대상 컬럼 숫자
      "<div style='height:47%; padding-bottom: 5px;'>" +
      "<div style='border-bottom: 1px solid #c5c5c5;border-top: 1px solid #c5c5c5;'>" +
      this.Lang.Dialog.NumericColumn +
      '</div>' +
      "<div id='" +
      name +
      "_NumberCols' name='NumberCols' class='" +
      this.Style +
      "PivotStandards' style='overflow: auto; height:calc(100% - 44px);padding:7px;border:1px solid #c5c5c5;'> " +
      ...
      '_PivotStandards ' +
      this.Style +
      "PivotStandards' style='height:calc(100% - 46px);padding:7px; overflow: auto;border:1px solid #c5c5c5;'>" +
      ...
      name +
      "_btns'>" +
      "<label style='cursor:pointer; float: left; padding-top: 3px;' for='layoutUpdate'><input style='cursor:pointer;' type='checkbox' id='layoutUpdate' />" +
      this.Lang.Dialog.PivotLayOutUpdate +
      '</label>' +
      (useStorage
        ? "<label style='cursor:pointer; padding-top: 3px;' for='" +
          name +
          "_saveStorage'><input type='checkbox' id='" +
          name +
          "_saveStorage'" +
          (saveInfo ? ' checked ' : '') +
          '>' +
          this.Lang.Dialog.SavePivotConfig +
          '</label>'
        : '') +
      "<button id='" +
      name +
      ...
      name +
      "_createPivotBtn'>" +
      this.Lang.Dialog.CreatePivot +
      "</button><button id='" +
      name +
      "_cancelBtn'>" +
      this.Lang.MenuButtons.Close +
      '</button></div>'
    ...
    if (!result) return
    if (self.PivotSheet && self.PivotSheet.MainTag.style.display == 'none')
      GetElem('SwitchBtn').textContent = this.Lang.Dialog.ShowPivot
    ...
    var T = this
    if (GetElem('layoutUpdate').checked)
      document.querySelector('.SheetDisabled').style.opacity = 0.2
    ...
    if (!pivotBodyDiv && this.Document)
      pivotBodyDiv = this.Document.querySelectorAll('.' + themePrefix + classDlg)[0]
    ...
    function addEventListenersToChildren(parentElement) {
      for (var i = 0; i < parentElement.children.length; i++) {
        ...
        parentElement.children[i].oncontextmenu = function (event) {
          if (
            this.parentElement != GetElem(name + '_NomalCols') &&
            this.parentElement != GetElem(name + '_NumberCols')
          )
            this.remove()
    ...
    addEventListeners(searchInput, {
      oninput: function (event) {
        ...
        if (!nomalCols && self.Document)
          nomalCols = self.Document.querySelectorAll('[name="NomalCols"]')[0]
        var nomColList = nomalCols.querySelectorAll('b')
        var numCols = document.getElementsByName('NumberCols')[0]
        if (!numCols && self.Document)
          numCols = self.Document.querySelectorAll('[name="NumberCols"]')[0]
    ...
    addEventListeners(pivotBodyDiv, {
      ...
      onkeyup: (event) => {
        T.closePivotDialog(event)
      },
      onmouseup: (event) => {
        T.PivotDragMouseUp(event)
      },
      onmousemove: (event) => {
        T.PivotDragMouseMove(event)
      },
      onclick: (event) => {
        if (!event.target.id && event.target.tagName != 'INPUT') T.closeDialog()
      },
    })
    ...
    addEventListeners(pivotRow, {
      onmouseup: function (event) {
        T.PivotDragSetItem(this, event, 1)
      },
      onscroll: (event) => {
        T.closeDialog()
      },
    ...
    addEventListeners(pivotData, {
      onmouseup: function (event) {
        T.PivotDragSetItem(this, event, 2)
      },
      onscroll: (event) => {
        T.closeDialog()
      },
      onchange: (event) => {
        if (layoutUpdate.checked) GetElem(name + '_createPivotBtn').click()
      },
    })
    ...
    btnCreate.onclick = () => {
      ...
      var dialogsArea =
        (self.DialogsArea ? self.DialogsArea : document) ||
        (IBSheet.DialogsArea ? IBSheet.DialogsArea : document)
      ...
      if (layoutUpdate.checked) {
        if (targetPFilter.length == 0) {
          ...
          for (var flt in self.PivotFilterDialog)
            if (targetPFilter.indexOf(flt) === -1) delete self.PivotFilterDialog[flt]
        }
      }
      ...
      setTimeout(() => {
        ...
        var init = {
          ...
          filterCols:
            self.PivotFilterCols && typeof self.PivotFilterCols != 'string'
              ? self.PivotFilterCols.join(',')
              : targetPFilter.join(','),
        }
    ...
    btnClear.onclick = () => {
      setTimeout(() => {
        ...
        delete this.PivotRows
        delete this.PivotCols
        delete this.PivotData
        delete this.PivotFunc
        delete this.PivotFiltering
        delete this.PivotFilterDialog
        this.PivotSheet && this.PivotSheet.clearPivotSheet()
      }, 10)
      this.closeDialog()
      ...
      for (var i = 0; i < clrArea.length; i++) {
        ...
        while (target.firstChild) {
          document
            .getElementById('pivotDialog_chk_' + target.firstChild.firstChild.getAttribute('id'))
            .removeAttribute('checked')
          document.getElementById(
            'pivotDialog_chk_' + target.firstChild.firstChild.getAttribute('id'),
          ).checked = false
  ...
  Fn.findPivotColumn = function () {
    ...
    var cols = this.getCols().filter(
      function (col) {
        return (
          col !== 'SEQ' &&
          excludeCol.indexOf(this.Cols[col].Type) === -1 &&
          this.Cols[col].Visible === 1
        )
      }.bind(this),
    )
    for (var i = 0; i < cols.length; i++) {
      this.Cols[cols[i]].Type == 'Int' || this.Cols[cols[i]].Type == 'Float'
        ? res.number.push({
            Name: cols[i],
            Value: this.getHeaderRows()
              .filter((r) => r.Visible)
              .map((r) => r[cols[i]])
              .join('/'),
          })
  ...
  Fn.PivotDragExactTarget = (target, group) =>
    group.filter((col) => target.indexOf(col.Name) > -1).length == target.length
  ...
  Fn.PivotDragSetItem = function (object, ev) {
    ...
    if (drag.tag) {
      ...
      for (var i = 0; i < tDataArr.length; i++)
        if (tDataArr && tDataArr[i].id == fromDragTagName) istoTarget = false

      if (
        toTarget.tagName == 'SELECT' ||
        (toTarget.tagName == 'INPUT' && toTarget.getAttribute('type') != 'checkbox')
      ) {
      ...
      if (
        fromDragArea == toTarget &&
        (toTarget == targetPRow ||
          toTarget == targetPCol ||
          toTarget == targetPData ||
          toTarget == targetPFilter)
      ) {
        ...
        fromDragArea == toTarget ||
        (fromDragArea == nomalColsArea && toTarget == numberColsArea) ||
        (fromDragArea == numberColsArea && toTarget == nomalColsArea) ||
        toTarget.tagName == 'INPUT' ||
        toTarget.tagName == 'SELECT'
      )
        return false
      ...
      if (toTarget == targetPFilter || toTarget == targetPRow || toTarget == targetPCol) {
        ...
        if (fromDragArea == nomalColsArea || fromDragArea == numberColsArea) {
          ...
          if (
            fromDragArea == targetPData &&
            (toTarget == targetPRow || toTarget == targetPCol) &&
            istoTarget
          ) {
            for (var i = 0; i < tFilterArr.length; i++) {
              if (fromDragTagName == tFilterArr[i].id) {
        ...
        toTarget == targetPData &&
        (fromDragArea == nomalColsArea || fromDragArea == numberColsArea)
      ) {
        for (var i = 0; i < tColArr.length; i++) {
          if (fromDragTagName == tColArr[i].id) {
            this.alert(pivDup.replace('%d', pivRowStr + ', ' + pivColStr + ', ' + pivDataStr))
      ...
      if (object.className.indexOf('_PivotStandards') > -1) {
        ...
        if (toTarget == targetPFilter && dragselect) {
          ...
          if (draginput) {
            if (self.PivotFilterDialog && self.PivotFilterDialog[fromDragTagName])
              draginput.value = this.Lang.Dialog.PivotSelected
      ...
      if (
        fromDragArea != nomalColsArea &&
        fromDragArea != numberColsArea &&
        object.id != fromDragTagId
      ) {
        drag.ClearDragObj(1, this)
        this.closeDialog()
  ...
  Fn.PivotItemCheckboxClick = function (tag, ev) {
    ...
    function makeClickObj(object, ev, T) {
      ...
      this.orgTag = object
      this.fromTagId =
        ev.target.nextSibling.tagName == 'B'
          ? ev.target.nextSibling.parentElement.parentElement.id
          : ev.target.nextSibling.tagName == 'SPAN'
            ? ev.target.nextSibling.parentElement.id
            : ev.target.nextSibling.id

      if (
        ev.target &&
        ev.target.parentElement &&
        ev.target.parentElement.childNodes[1] &&
        !(
          ev.target.parentElement.childNodes[0].tagName == 'INPUT' &&
          ev.target.parentElement.childNodes[0].getAttribute('type') == 'checkbox'
        )
      ) {
        var orgSel = T.Document.querySelectorAll(
          "[name='" + ev.target.parentElement.childNodes[1].name + "']",
        )[0]
        var divSel = div.childNodes[0].childNodes[0].childNodes[1]
        var options = divSel.getElementsByTagName('option')
  ...
  Fn.producePivotColumn = function () {
    ...
    var T = this
    var arPivotRows = this.PivotRows
        ? this.PivotRows.split(',')
        : this.InitPivotRows
          ? this.InitPivotRows.split(',')
          : [],
      arPivotCols = this.PivotCols
        ? this.PivotCols.split(',')
        : this.InitPivotCols
          ? this.InitPivotCols.split(',')
          : [],
      arPivotData = this.PivotData
        ? this.PivotData.split(',')
        : this.InitPivotData
          ? this.InitPivotData.split(',')
          : []
    ...
    if (cols.common && cols.common.length > 0) {
      for (var i = 0; i < arPivotRows.length; i++) {
        _IBSheet.__.find(cols.common, (v) => {
          if (v.Name == arPivotRows[i]) {
            strCols =
              "<span><b style='color:#586980;' class='box' id='" +
              v.Name +
              "'>" +
              v.Value +
              makeSelString(v.Name, i, 'normal') +
              '</b></span>'
      ...
      for (var i = 0; i < cols.common.length; i++) {
        ...
        if (this.PivotFilterDialog && this.PivotFilterDialog[cols.common[i].Name]) {
          strColsFilter =
            "<span style='z-index:0;'><b style='color:#586980;' class='box' id='" +
            cols.common[i].Name +
            "'>" +
            cols.common[i].Value +
            makeSelString(cols.common[i].Name, i, 'normal', '', 1) +
            '</b></span>'
        ...
        strCols =
          "<span style='z-index:0;'><input type='checkbox' " +
          (isUse == true ? "checked='true'" : '') +
          "id='pivotDialog_chk_" +
          cols.common[i].Name +
          "'></input><b style='color:#586980;' class='box' id='" +
          cols.common[i].Name +
          "'>" +
          cols.common[i].Value +
          makeSelString(cols.common[i].Name, i, 'normal') +
          '</b></span>'
        strCommon += strCols
      }
    }
    ...
    function makeSelString(cName, idx, category, hidden, filter) {
      ...
      if (T['PivotData'] && idx !== '')
        var isData = T['PivotData'].split(',').indexOf(cName) != -1 ? true : false
      function getOptSelected(value) {
        return value === selectedValue ? ' selected' : ''
      }
      var res = ''
      if (category == 'num') {
        res +=
          "<select class='dataSelBox' tabindex='-1' style='padding-left: 1px; text-align: center; display:" +
          (hidden ? "none;'" : "'") +
          "name='" +
          cName +
          "'>"
        res +=
          "<option style='font-size:10px;' value='Sum'" +
          getOptSelected('Sum') +
          '>' +
          T['Lang']['Text']['PivotSum'] +
          '</option>'
        res +=
          "<option style='font-size:10px;' value='Avg'" +
          getOptSelected('Avg') +
          '>' +
          T['Lang']['Text']['PivotAvg'] +
          '</option>'
        res +=
          "<option style='font-size:10px;' value='Min'" +
          getOptSelected('Min') +
          '>' +
          T['Lang']['Text']['PivotMin'] +
          '</option>'
        res +=
          "<option style='font-size:10px;' value='Max'" +
          getOptSelected('Max') +
          '>' +
          T['Lang']['Text']['PivotMax'] +
          '</option>'
        res +=
          "<option style='font-size:10px;' value='Count'" +
          getOptSelected('Count') +
          '>' +
          T['Lang']['Text']['PivotCount'] +
          '</option>'
        res += '</select>'
      } else {
        res +=
          "<select class='dataSelBox' tabindex='-1' style='display:" +
          (isData && hidden == 0 ? "'" : 'none;') +
          "' name='" +
          cName +
          "'>"
        res +=
      ...
      if (
        T.Cols[cName] &&
        !(
          T.Cols[cName].Type == 'Link' ||
          T.Cols[cName].Type == 'Html' ||
          T.Cols[cName].Type == 'File' ||
          T.Cols[cName].Type == 'Pass'
        )
      ) {
        res +=
          "<input class='filBtn' value='" +
          (T.PivotFilterDialog && T.PivotFilterDialog[cName] != ''
            ? T['Lang']['Dialog']['PivotSelected']
            : T['Lang']['Dialog']['PivotAll']) +
          "' onclick='" +
          T.id +
          ".ShowPivotFilter(this, event);' name='" +
          cName +
          "' type='button'" +
          (category != 'filter' && filter != 1 ? " style='display:none;'" : '') +
          ' />'
      }
      return res
    }
    ...
    if (cols.number && cols.number.length > 0) {
      for (var i = 0; i < arPivotRows.length; i++) {
        _IBSheet.__.find(cols.number, (v) => {
          if (v.Name == arPivotRows[i]) {
            strCols =
              "<span><b style='color:#586980;' class='box' id='" +
              v.Name +
        ...
        _IBSheet.__.find(cols.common, (v) => {
          if (v.Name == arPivotData[i]) {
            strCols =
              ...
              "'>" +
              v.Value +
              makeSelString(v.Name, i, 'normal', 0) +
              '</b></span>'
            strPivotData += strCols
          }
        })
      ...
      for (var i = 0; i < cols.number.length; i++) {
        ...
        if (this.PivotFilterDialog && this.PivotFilterDialog[cols.number[i].Name]) {
          strColsFilter =
            ...
            "'>" +
            cols.number[i].Value +
            makeSelString(cols.number[i].Name, '', 'filter', 1) +
            '</b></span>'
          strPivotFilter += strColsFilter
          isUse = true
        }
        ...
        strCols =
          ...
          "'>" +
          cols.number[i].Value +
          makeSelString(cols.number[i].Name, i, 'num', 1) +
          '</b></span>'
        strNum += strCols
      }
    }
