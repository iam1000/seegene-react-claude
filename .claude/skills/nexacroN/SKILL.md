...
| MultiCombo | 다중 선택 드롭다운 | `mcbo_` |
| CheckBoxSet | 체크박스 그룹 (다중 선택) | `chks_` |
...
<MultiCombo id="mcbo_area" innerdatasetid="ds_areaCd" codecolumn="cd" datacolumn="cdNm" edittype="tag"/>
<CheckBoxSet id="chks_option" innerdatasetid="ds_optionCd" codecolumn="cd" datacolumn="cdNm" columncount="3"/>
```
...
## 11. 컴포넌트별 상세 API 레퍼런스

CHM 공식 매뉴얼에서 추출한 컴포넌트별 Property/Method/Event 전체 목록이
`component/` 디렉토리에 개별 skill로 정리되어 있다.
컴포넌트의 세부 속성, 메서드 시그니처, 파라미터, 반환값, 이벤트 핸들러 정보가 필요할 때 참조한다.

### 핵심 컴포넌트 (P0)

| Skill | 포함 컴포넌트 | 레퍼런스 경로 |
|-------|-------------|--------------|
| nexacron-grid | Grid, GridCellControl, GridBandControl 등 | `component/nexacron-grid/` |
| nexacron-dataset | Dataset, DataObject, ColumnInfo | `component/nexacron-dataset/` |
| nexacron-edit | Edit, MaskEdit, TextArea, Spin | `component/nexacron-edit/` |
| nexacron-combo | Combo, MultiCombo, ListBox, CheckBox, CheckBoxSet, Radio | `component/nexacron-combo/` |
| nexacron-button-static | Button, Static, ImageViewer, ProgressBar | `component/nexacron-button-static/` |

### 주요 컴포넌트 (P1)

| Skill | 포함 컴포넌트 | 레퍼런스 경로 |
|-------|-------------|--------------|
| nexacron-calendar | Calendar | `component/nexacron-calendar/` |
| nexacron-container | Div, PopupDiv, Tab, View, GroupBox | `component/nexacron-container/` |
| nexacron-listview | ListView, ListViewBandControl 등 | `component/nexacron-listview/` |
| nexacron-menu | Menu, PopupMenu | `component/nexacron-menu/` |
| nexacron-file | FileUpload, FileDownload, FileDialog, VirtualFile 등 | `component/nexacron-file/` |
| nexacron-form | Form, Layout, StepControl | `component/nexacron-form/` |

### 보조 컴포넌트 (P2)

| Skill | 포함 컴포넌트 | 레퍼런스 경로 |
|-------|-------------|--------------|
| nexacron-application | Application, Environment, Screen | `component/nexacron-application/` |
| nexacron-frames | MainFrame, ChildFrame, FrameSet 등 | `component/nexacron-frames/` |
| nexacron-graphics | Graphics, Sketch | `component/nexacron-graphics/` |
| nexacron-web | WebBrowser, WebView, GoogleMap, VideoPlayer, Plugin | `component/nexacron-web/` |
| nexacron-excel | ExcelExportObject, ExcelImportObject | `component/nexacron-excel/` |

### 확장 기능 (P3)

| Skill | 포함 컴포넌트 | 레퍼런스 경로 |
|-------|-------------|--------------|
| nexacron-communication | TCPClientSocket, XPush, NexacroAPI | `component/nexacron-communication/` |
| nexacron-script-objects | Date, Decimal, DomParser, Rect, Point 등 | `component/nexacron-script-objects/` |
| nexacron-device | Camera, Geolocation, BluetoothLE 등 20개 DeviceAPI | `component/nexacron-device/` |
| nexacron-events | EventObject, BindItem, System | `component/nexacron-events/` |

### 레퍼런스 사용법

각 skill 디렉토리의 구조:
```
component/nexacron-{name}/
├── SKILL.md                         # 개요 + 주요 API 요약
├── references/
│   ├── {Component}_properties.md    # 전체 Property 상세
│   ├── {Component}_methods.md       # 전체 Method 상세 (시그니처, 파라미터, 반환값)
│   ├── {Component}_events.md        # 전체 Event 상세 (핸들러 시그니처, EventInfo)
│   ├── {Component}_sub_objects.md   # 하위 객체 요약 (Grid, ListView 등)
│   └── sub_objects/                 # 하위 객체별 상세
└── evals/
    └── evals.json                   # 평가 테스트 케이스
```

컴포넌트 API를 조회할 때:
1. 해당 컴포넌트가 어느 skill에 속하는지 위 표에서 확인
2. `component/nexacron-{name}/SKILL.md`에서 개요 확인
3. 상세 정보는 `references/{Component}_properties.md` 등에서 확인
