...
16. [MultiCombo](#16-multicombo)
17. [CheckBoxSet](#17-checkboxset)

**★☆☆ 선택**
18. [Spin](#18-spin)
19. [ListBox](#19-listbox)
20. [ListView](#20-listview)
21. [ProgressBar](#21-progressbar)
22. [ImageViewer](#22-imageviewer)
23. [Menu](#23-menu)
24. [PopupMenu](#24-popupmenu)
25. [WebBrowser](#25-webbrowser)
26. [FileDialog](#26-filedialog)
27. [FileUpTransfer](#27-fileuptransfer)
...
| 이벤트 | 설명 |
|--------|------|
| `canitemchange` | 아이템 변경 전 (false 반환 시 변경 취소) |
| `onitemchanged` | 아이템 변경 완료 후 |
| `onitemclick` | 항목 클릭 시 |
...
<Combo id="cbo_gender" taborder="0" left="100" top="32" width="120" height="32"
       ...
       onitemchanged="cbo_gender_onitemchanged">
...
여러 항목을 동시에 선택할 수 있는 다중 선택 드롭다운 컴포넌트. Combo의 확장형.

> 상세 API는 `component/nexacron-combo/references/MultiCombo_*.md` 참조

### 핵심 속성

| 속성 | 설명 |
|------|------|
| `value` | 선택된 코드값 (구분자로 연결된 문자열) |
| `codecolumn` | 코드 컬럼 ID |
| `datacolumn` | 표시 텍스트 컬럼 ID |
| `innerdataset` | 아이템 리스트 Dataset |
| `separator` | 다중 선택 값 구분자 (기본: `,`) |
| `edittype` | 선택 표시 방식 (`tag`/`text`/`count`) |
| `counttextformat` | edittype이 `count`일 때 표시 형식 |
| `displayrowcount` | 드롭다운 표시 행 수 |
| `readonly` | 읽기 전용 여부 |
...
| 메서드 | 설명 |
|--------|------|
| `set_value(codes)` | 코드값 설정 (구분자로 연결) |
| `getCount()` | 전체 항목 수 반환 |
| `clearSelect()` | 선택 초기화 |
| `dropdown()` | 드롭다운 리스트 열기 |
...
```xml
<MultiCombo id="mcbo_area" left="100" top="10" width="250" height="32"
            innerdatasetid="ds_areaCd" codecolumn="cd" datacolumn="cdNm"
            edittype="tag" separator=","
            onitemchanged="mcbo_area_onitemchanged"/>
```

### Script 사용 예시

```javascript
// 다중 선택 값 읽기 (구분자로 연결된 문자열)
var sValues = this.mcbo_area.value;  // "01,03,05"

// 선택 값 설정
this.mcbo_area.set_value("01,02");

// 선택 변경 이벤트
this.mcbo_area_onitemchanged = function(obj:nexacro.MultiCombo, e:nexacro.ItemChangeEventInfo)
{
    trace("선택된 값: " + obj.value);
};
```
...
여러 개의 체크박스를 그룹으로 묶어 다중 선택을 지원하는 컴포넌트.

> 상세 API는 `component/nexacron-combo/references/CheckBoxSet_*.md` 참조
...
| `innerdataset` | 아이템 리스트 Dataset |
| `separator` | 다중 선택 값 구분자 (기본: `,`) |
| `columncount` | 한 행에 표시할 아이템 수 |
| `direction` | 아이템 배치 방향 (horizontal/vertical) |

### 핵심 이벤트

| 이벤트 | 설명 |
|--------|------|
| `canitemchange` | 아이템 변경 전 |
| `onitemchanged` | 아이템 변경 완료 후 |
| `onitemclick` | 아이템 클릭 시 |

### XFDL 선언 예시

```xml
<CheckBoxSet id="chks_option" left="100" top="10" width="400" height="60"
             innerdatasetid="ds_optionCd" codecolumn="cd" datacolumn="cdNm"
             columncount="3" direction="horizontal"
             onitemchanged="chks_option_onitemchanged"/>
```
...
```javascript
// 다중 선택 값 읽기
var sValues = this.chks_option.value;  // "OPT01,OPT03"

// 선택 변경 이벤트
this.chks_option_onitemchanged = function(obj:nexacro.CheckBoxSet, e:nexacro.ItemChangeEventInfo)
{
    trace("선택된 항목: " + obj.value);
};
```
