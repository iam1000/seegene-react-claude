# MultiComboListControl Properties
...
| `-nexa-rtl-edge-image` | MultiComboListControl 의 rtl 속성이 true 로 적용될 때 테두리(Border) 안쪽에 표시될 Edge 이미지를 설정하는  |
...
**Setting Syntax**
```javascript
MultiComboListControl 영역의 안쪽 여백을 pixel 단위로 설정합니다.
...
**Syntax**
```javascript
MultiComboListControl.set_textAlign(strtextalign) // set
MultiComboListControl.textAlign // get
```
...
**Syntax**
```javascript
MultiComboListControl.set_wordWrap(strwordwrap) // set
MultiComboListControl.wordWrap // get
```
...
**Syntax**
```javascript
MultiComboListControl.set_boxShadow(strbshadow) // set
MultiComboListControl.boxShadow // get
```
...
**Syntax**
```javascript
MultiComboListControl.cursor // get
```
