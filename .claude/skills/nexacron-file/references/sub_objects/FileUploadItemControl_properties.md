# FileUploadItemControl Properties
...
| `-nexa-rtl-edge-image` | FileUploadItemControl 의 rtl 속성이 true 로 적용될 때 테두리(Border) 안쪽에 표시될 Edge 이미지를 설정하는  |
...
**Setting Syntax**
```javascript
FileUploadItemControl 영역의 안쪽 여백을 pixel 단위로 설정합니다.
...
**Syntax**
```javascript
FileUploadItemControl.cursor // get
```
