# PopupMenuItemControl Properties
...
| `-nexa-rtl-edge-image` | PopupMenuItemControl 의 rtl 속성이 true 로 적용될 때 테두리(Border) 안쪽에 표시될 Edge 이미지를 설정하는 속 |
...
**Setting Syntax**
```javascript
PopupMenuItemControl 에서 텍스트와 테두리 사이의 여백을 pixel 단위로 설정합니다.
...
**Syntax**
```javascript
PopupMenuItemControl.cursor // get
```
