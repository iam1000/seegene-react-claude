...
**Syntax**
```javascript
Plugin.set_border(strborder) // set
Plugin.border // get
```
...
**Syntax**
```javascript
Plugin.set_adjustalpha(bAdjustalpha) // set
Plugin.adjustalpha // get
```

**Setting Syntax**
```javascript
"true" 설정 시 alpha 값을 다시 조절합니다.
...
**Syntax**
```javascript
Plugin.set_archive(strArchive) // set
Plugin.archive // get
```

**Setting Syntax**
```javascript
플러그인 archive 속성값을 설정합니다.
```
...
**Syntax**
```javascript
Plugin.set_bottom(nBottom) // set
Plugin.bottom // get
```

**Setting Syntax**
```javascript
Plugin 의 하단 좌표값을 pixel 또는 비율("%") 단위의 숫자로 설정합니다.

* <strCompID> 값을 생략했을 때 :
pixel 단위로 설정 시 부모(Parent)의 bottom 속성값을 기준으로 Plugin 의 하단 위치가 결정됩니다.
비율("%") 단위로 설정 시 부모(Parent)의 height 속성값을 기준으로 Plugin 의 하단 위치가 결정됩니다.

* <strCompID> 값을 설정했을 때 :
pixel 단위로 설정 시 <strCompID> 의 top 속성값을 기준으로 Plugin 의 하단 위치가 결정됩니다.
비율("%") 단위로 설정 시 <strCompID> 의 height 속성값을 기준으로 Plugin 의 하단 위치가 결정됩니다.
```
...
**Syntax**
```javascript
Plugin.set_classid(strClassid) // set
Plugin.classid // get
```

**Setting Syntax**
```javascript
Plugin 에 연결할 객체의 classid 또는 progid 를 문자열로 설정합니다.
```
...
**Syntax**
```javascript
Plugin.set_classid64(strClassid64) // set
Plugin.classid64 // get
```

**Setting Syntax**
```javascript
Plugin 에 연결할 64bit 객체의 classid 또는 progid 를 문자열로 설정합니다.
```
...
**Syntax**
```javascript
Plugin.set_code(strCode) // set
Plugin.code // get
```

**Setting Syntax**
```javascript
Plugin 에 연결할 객체의 code 를 문자열로 설정합니다.
```
...
**Syntax**
```javascript
Plugin.set_codebase(strUrl) // set
Plugin.codebase // get
```

**Setting Syntax**
```javascript
Plugin 에 연결할 객체의 위치 경로를 문자열로 설정합니다.
```
...
- IE 계열 브라우저는 Form 로드 이후에 동적으로 codebase 속성값을 설정하면 설치팝업이 표시되지 않습니다.
  > IE10 이하는 넥사크로 스튜디오에서 codebase 속성값을 설정하면 Form 로드 시 설치팝업이 나타납니다.
  > IE11 이상은 mimetype 속성값을 "application/x-shockwave-flash" 로 설정해야 Form 로드 시점에 설치팝업이 나타납니다.

- Chrome, Firefox, Edge 등 타 브라우저는 Form 로드 시점에도 설치팝업이 표시되지 않습니다.
...
**Syntax**
```javascript
Plugin.set_contents(strContents) // set
Plugin.contents // get
```

**Setting Syntax**
```javascript
속성 또는 메타 데이터를 문자열로 설정합니다.
```
...
**Syntax**
```javascript
Plugin.set_enable(benable) // set
Plugin.enable // get
```

**Setting Syntax**
```javascript
Plugin 을(를) 사용할 수 없게 설정합니다.
...
**Syntax**
```javascript
Plugin.set_flexshrink(fVal) // set
Plugin.flexshrink // get
```
...
**Syntax**
```javascript
Plugin.set_height(nHeight) // set
Plugin.height // get
```

**Setting Syntax**
```javascript
Plugin 의 높이를 pixel 또는 비율("%") 단위의 숫자로 설정합니다.

* <strCompID> 값을 생략했을 때 :
pixel 단위로 설정 시 설정값이 높이로 결정됩니다.
비율("%") 단위로 설정 시 부모(Parent)의 height 속성값을 기준으로 Plugin 의 높이가 결정됩니다.

* <strCompID> 값을 설정했을 때 :
pixel 단위로 설정 시 <strCompID> 값은 무시되고 설정값이 높이로 결정됩니다.
비율("%") 단위로 설정 시 <strCompID> 의 height 속성값을 기준으로 Plugin 의 높이가 결정됩니다.
```
...
**Syntax**
```javascript
Plugin.set_initvalueid(strValueID) // set
Plugin.initvalueid // get
```

**Setting Syntax**
```javascript
InitValueDefinition 영역에 정의된 InitValue 파일에서 Plugin 에 적용할 항목의 ID 를 문자열로 설정합니다.
```
...
**Syntax**
```javascript
Plugin.set_layoutorder(nVal) // set
Plugin.layoutorder // get
```
...
**Syntax**
```javascript
Plugin.set_left(nLeft) // set
Plugin.left // get
```

**Setting Syntax**
```javascript
Plugin 의 좌측 좌표값을 pixel 또는 비율("%") 단위의 숫자로 설정합니다.

* <strCompID> 값을 생략했을 때 :
pixel 단위로 설정 시 부모(Parent)의 left 속성값을 기준으로 Plugin 의 좌측 위치가 결정됩니다.
비율("%") 단위로 설정 시 부모(Parent)의 width 속성값을 기준으로 Plugin 의 좌측 위치가 결정됩니다.

* <strCompID> 값을 설정했을 때 :
pixel 단위로 설정 시 <strCompID> 의 right 속성값을 기준으로 Plugin 의 좌측 위치가 결정됩니다.
비율("%") 단위로 설정 시 <strCompID> 의 width 속성값을 기준으로 Plugin 의 좌측 위치가 결정됩니다.
```
...
**Syntax**
```javascript
Plugin.set_license(strLicense) // set
Plugin.license // get
```
...
**Syntax**
```javascript
Plugin.set_lpkpath(strlpkPath) // set
Plugin.lpkpath // get
```

**Setting Syntax**
```javascript
객체의 lpkpath 속성값을 설정합니다.
```
...
**Syntax**
```javascript
Plugin.set_maxheight(nMaxHeight) // set
Plugin.maxheight // get
```

**Setting Syntax**
```javascript
Plugin 이(가) 화면에 표시되는 높이의 최대값을 pixel 단위의 숫자로 설정합니다.
```
...
**Syntax**
```javascript
Plugin.set_maxwidth(nMaxWidth) // set
Plugin.maxwidth // get
```

**Setting Syntax**
```javascript
Plugin 이(가) 화면에 표시되는 너비의 최대값을 pixel 단위의 숫자로 설정합니다.
```
...
**Syntax**
```javascript
Plugin.set_mimetype(strMIMEType) // set
Plugin.mimetype // get
```
...
**Syntax**
```javascript
Plugin.set_minheight(nMinHeight) // set
Plugin.minheight // get
```

**Setting Syntax**
```javascript
Plugin 이(가) 화면에 표시되는 높이의 최소값을 pixel 단위의 숫자로 설정합니다.
```
...
**Syntax**
```javascript
Plugin.set_minwidth(nMinWidth) // set
Plugin.minwidth // get
```

**Setting Syntax**
```javascript
Plugin 이(가) 화면에 표시되는 너비의 최소값을 pixel 단위의 숫자로 설정합니다.
```
...
**Syntax**
```javascript
Plugin.set_pluginpage(strPluginPage) // set
Plugin.pluginpage // get
```
...
**Syntax**
```javascript
Plugin.set_pluginsrc(strPluginSrc) // set
Plugin.pluginsrc // get
```

**Setting Syntax**
```javascript
객체의 data, src 속성값을 설정합니다.
```
...
**Syntax**
```javascript
Plugin.set_popupstyle(bPopupStyle) // set
Plugin.popupstyle // get
```

**Setting Syntax**
```javascript
true 설정 시 window 를 팝업 스타일로 생성합니다.
...
**Syntax**
```javascript
Plugin.set_positionstep(nStep) // set
Plugin.positionstep // get
```

**Setting Syntax**
```javascript
Plugin 이(가) 표시될 분할화면의 인덱스를 설정합니다.
...
**Syntax**
```javascript
Plugin.set_right(nRight) // set
Plugin.right // get
```

**Setting Syntax**
```javascript
Plugin 의 우측 좌표값을 pixel 또는 비율("%") 단위의 숫자로 설정합니다.

* <strCompID> 값을 생략했을 때 :
pixel 단위로 설정 시 부모(Parent)의 right 속성값을 기준으로 Plugin 의 우측 위치가 결정됩니다.
비율("%") 단위로 설정 시 부모(Parent)의 width 속성값을 기준으로 Plugin 의 우측 위치가 결정됩니다.

* <strCompID> 값을 설정했을 때 :
pixel 단위로 설정 시 <strCompID> 의 left 속성값을 기준으로 Plugin 의 우측 위치가 결정됩니다.
비율("%") 단위로 설정 시 <strCompID> 의 width 속성값을 기준으로 Plugin 의 우측 위치가 결정됩니다.
```
...
**Syntax**
```javascript
Plugin.set_tablecellarea(strArea) // set
Plugin.tablecellarea // get
```
...
**Syntax**
```javascript
Plugin.set_taborder(nTabOrder) // set
Plugin.taborder // get
```
...
**Syntax**
```javascript
Plugin.set_tabstop(bTabStop) // set
Plugin.tabstop // get
```

**Setting Syntax**
```javascript
Plugin 이(가) 탭키 입력에 의한 포커스를 받지 않습니다.
```
...
**Syntax**
```javascript
Plugin.set_top(nTop) // set
Plugin.top // get
```

**Setting Syntax**
```javascript
Plugin 의 상단 좌표값을 pixel 또는 비율("%") 단위의 숫자로 설정합니다.

* <strCompID> 값을 생략했을 때 :
pixel 단위로 설정 시 부모(Parent)의 top 속성값을 기준으로 Plugin 의 상단 위치가 결정됩니다.
비율("%") 단위로 설정 시 부모(Parent)의 height 속성값을 기준으로 Plugin 의 상단 위치가 결정됩니다.

* <strCompID> 값을 설정했을 때 :
pixel 단위로 설정 시 <strCompID> 의 bottom 속성값을 기준으로 Plugin 의 상단 위치가 결정됩니다.
비율("%") 단위로 설정 시 <strCompID> 의 height 속성값을 기준으로 Plugin 의 상단 위치가 결정됩니다.
```
...
**Syntax**
```javascript
Plugin.set_usepersistdata(bUsePersistData) // set
Plugin.usepersistdata // get
```

**Setting Syntax**
```javascript
"true" 설정 시 persistdata를 사용합니다.
...
**Syntax**
```javascript
Plugin.set_visible(bVisible) // set
Plugin.visible // get
```

**Setting Syntax**
```javascript
Plugin 을(를) 화면에 표시하지 않습니다.
```
...
**Syntax**
```javascript
Plugin.set_width(nWidth) // set
Plugin.width // get
```

**Setting Syntax**
```javascript
Plugin 의 너비를 pixel 또는 비율("%") 단위의 숫자로 설정합니다.

* <strCompID> 값을 생략했을 때 :
pixel 단위로 설정 시 설정값이 너비로 결정됩니다.
비율("%") 단위로 설정 시 부모(Parent)의 width 속성값을 기준으로 Plugin 의 너비가 결정됩니다.

* <strCompID> 값을 설정했을 때 :
pixel 단위로 설정 시 <strCompID> 값은 무시되고 설정값이 너비로 결정됩니다.
비율("%") 단위로 설정 시 <strCompID> 의 width 속성값을 기준으로 Plugin 의 너비가 결정됩니다.
```
...
**Syntax**
```javascript
Plugin.set_windowed(bWindowed) // set
Plugin.windowed // get
```

**Setting Syntax**
```javascript
true 설정 시 window 를 갖는 플러그인으로 설정됩니다.
