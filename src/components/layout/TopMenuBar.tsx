...
const THEME = {
  ...
  // ── Concept J : 기존2 (빨간 상단 메뉴바) ──────────────────────────────
  J: {
    wrap: 'bg-[#D33D44] text-white',
    title: 'border-r border-[#e05560]',
    hover: 'hover:bg-[#e05560]',
    active: 'bg-[#e05560]',
    right: 'border-l border-[#e05560] text-red-100',
    winHover: 'hover:bg-[#e05560]',
    winClose: 'hover:bg-[#b02830]',
  },
  ...
  return (
    <div className={`flex items-center h-8 text-xs select-none shrink-0 ${t.wrap}`}>
      <span className={`px-3 whitespace-nowrap font-medium ${t.title}`}>
        {layoutConcept === 'A' || layoutConcept === 'J'
          ? '재단법인 씨젠의료재단 LIS-QA'
          : '오픈헬스케어 LIS-QA'}
      </span>
      ...
      <div className={`flex items-center gap-1 px-3 text-[11px] ${t.right}`}>
