...
│   ├── ui/                 # shadcn/ui 원본 (CLI로 설치, 직접 수정 금지)
│   ├── common/             # shadcn 래퍼 + 프로젝트 전용 컴포넌트
│   │   │                   # Button, Input, Select, Checkbox, RadioGroup, TextArea
│   │   │                   # MaskEdit, Spin, WebBrowser, ImageViewer, PopupMenu, ListBox
│   ├── editor/             # RichEditor (TipTap — DEXT5Editor 대응)
...
> **Nexacro → React 변환 작업 시 `nexacro-to-react` 스킬이 자동 로드됩니다. (트리거: 변환, 전환, convert, xfdl 등)**
...
6. **UI 라이브러리 우선순위**: shadcn/ui → shadcn에 없거나 구현이 너무 복잡한 경우 MUI → 그 외 native HTML
